...
- the unsupported historical `record-event *_card_ack` path is rejected, not rerouted.
...
| 3 | Reject unsupported historical `record-event *_card_ack` submissions. | Unsupported historical with the old path keeps the system ambiguous. | Router returns a control-plane failure instead of accepting or rerouting the ACK. |
...
| --- | --- | --- |
| 1 | Controller receives, submits, or records a system-card ACK. | Unsupported historical `record-event *_card_ack` path is rejected and prompts forbid Controller ACK handling. |
