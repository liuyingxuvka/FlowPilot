...
| 3 | Allow worker or FlowGuard operator supplement/reissue as fresh work, not in-place mutation of old failed evidence. | PM guidance allows asking the original role to produce a new report/result based on the old artifact as context. The old blocked artifact remains stale context, not passing evidence. | Repair packets/results get a new generation and must be reviewed again before PM can complete the node. |
