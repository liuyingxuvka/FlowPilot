...
- PM, worker, reviewer, and FlowGuard operator flows must not rely on role memory or
...
- Existing valid flows such as current-node worker review, PM role-work
  requests, FlowGuard operator reports, material scan, research, resume decisions, and
...
| `pm_role_work_request` | `project_manager` | worker, reviewer, route-scope FlowGuard operator, product-scope FlowGuard operator | `pm.role_work_request` | `project_manager` | `project_manager` | `pm_records_role_work_result_decision` or a new PM-selected follow-up | `project_manager` |
| `flowguard_operator_model_report` | `project_manager` or Router gate | process/product-scope FlowGuard operator | `flowguard_operator.model_report` or `flowguard_operator.model_miss_report` | `project_manager` | `project_manager` | PM decision event that consumes the report | `project_manager` |
...
| `ordinary_material_evidence_work` | PM ordinary research/source-verification work package when direct reading is insufficient | worker, reviewer, or FlowGuard operator within the existing role-work boundary | existing research or PM role-work packet family | risk-appropriate reviewer or PM disposition | existing reviewer/PM path | existing research/role-work result and disposition events | the current research or PM role-work owner; no dedicated material authority |
...
| 2 | Build the safe binding table in the model | Model all common process kinds, not just PM role work. | Safe scenarios for current-node work, PM role-work, FlowGuard operator report, material scan, research, control-blocker repair, and resume pass. |
...
| R9 | FlowGuard operator model-miss requests are blocked because they are not PM role-work. | PM must still delegate model-miss analysis. | Safe FlowGuard operator-report scenario. |
...
| All roles use the same binding rule | Safe/hazard cases cover PM, worker, reviewer, and FlowGuard operator process kinds. | Router helper is generic and table-driven. |
...
| Valid existing flows still pass | Safe scenarios for current-node, PM role-work, FlowGuard operator, material, research, control-blocker, resume. | Existing focused tests still pass. |
