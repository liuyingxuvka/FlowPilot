...
Close the FlowPilot protocol gap where a reviewer or FlowGuard operator can legitimately
...
| 2 | Add a gate-outcome completeness check | Model every reviewer/FlowGuard operator gate as needing a pass path plus a non-pass path, unless the gate has an explicit structured report path that already carries sufficient/insufficient outcomes. | FlowGuard lists pass-only gate groups instead of only checking the one observed child-skill gate. |
...
| 4 | Implement standard block handling by repair class | Route block outcomes by class: PM rewrites source artifact, same-node repair/model-miss triage, FlowGuard operator route repair, or terminal/evidence repair. | A block records a file-backed report, clears stale pass state, and sends the next action to the correct repair owner. |
...
| B2 | A reviewer/FlowGuard operator gate has only a pass event and no non-pass route. | Any legitimate blocker becomes invisible or pressures the role to fake a pass. | Gate outcome completeness audit must flag pass-only review/FlowGuard operator event groups. |
...
| B5 | A block has no repair owner. | The route exits pass-only mode but then becomes stuck. | Model and tests must assert the next action routes to PM, FlowGuard operator repair, or controlled stop. |
| B6 | Repair rerun keeps stale pass evidence. | PM could fix an artifact while old reviewer/FlowGuard operator passes remain trusted. | Tests must assert affected pass flags and review artifacts are invalidated or superseded before recheck. |
...
| B8 | The model only catches the observed child-skill event. | The same protocol gap can recur at another gate. | Hazard cases must include at least one unseen pass-only reviewer gate and one FlowGuard operator gate. |
...
- route non-pass outcomes back to a PM or FlowGuard operator repair loop instead of
