...
| 2 | Product canonical or unsupported historical event sets only one side of the flags. | `product_behavior_model_alias_does_not_set_unsupported historical_flags` |
...
| 7 | Process canonical or unsupported historical event sets only one side of the flags. | `process_route_model_alias_does_not_set_unsupported historical_flags` |
...
7. Mirror unsupported historical artifacts:
   `flowguard/product_architecture_modelability.json` and
   `flowguard/route_process_check.json`.
8. Make downstream helpers prefer canonical paths with unsupported historical fallback.
