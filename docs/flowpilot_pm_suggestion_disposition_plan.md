...
Unify reviewer, worker, and FlowGuard operator suggestions under one
Project-Manager-owned disposition loop without flattening role authority.

Current state already routes reviewer findings, worker/FlowGuard operator `PM Note`
...
| 2 | Unified suggestion item | Define one `PM Suggestion Item` shape for reviewer, worker, process-FlowGuard operator, and product-FlowGuard operator suggestions. | Templates/cards use the same fields for source role, classification, evidence refs, PM disposition, and closure state. |
...
| 4 | Preserve authority differences | Keep reviewer/FlowGuard operator gate authority distinct from worker advisory notes. | Reviewer hard blockers can block only when minimum gate standard is not guaranteed; workers cannot block; formal FlowGuard operator model blockers stay gate-bearing. |
...
| 9 | Runtime/card propagation | Update reviewer cards, worker/FlowGuard operator packet templates, PM phase cards, and output contracts so suggestion items can be produced and consumed uniformly. | Card coverage and output-contract tests verify the new fields and authority language. |
...
| R4 | FlowGuard operator tool/model improvement suggestion blocks project completion. | FlowPilot maintenance work could derail the user’s current project. | `flowpilot_skill_improvement` dispositions remain nonblocking unless PM classifies a true current-project blocker separately. |
...
- suggestion source roles: reviewer, worker, route-scope FlowGuard operator, product-scope FlowGuard operator;
...
- authority constraints for reviewer, worker, and FlowGuard operator-origin items;
...
4. Add schema/template docs for `pm_suggestion_ledger.jsonl` and suggestion
   item entries.
5. Update PM/reviewer/worker/FlowGuard operator cards and packet templates to produce or
