...
| --- | --- | --- | --- |
| 1 | Define one recovery engine, not separate heartbeat and mid-run recovery paths. | Add a durable role-recovery transaction schema and route both `heartbeat_or_manual_resume_requested` and a new mid-run role liveness fault event through that transaction. | Recovery records carry `trigger_source`, `scope`, `target_role_keys`, `role_binding_generation`, and `priority=preempt_normal_work`. |
...
| 5 | Implement the recovery ladder. | For each failed role: try old-binding restore first; if restore fails, create a targeted replacement; if capacity/full-slot conflict prevents replacement, reconcile slots; if recovery cannot produce an addressable current-run binding, block for environment/user action. | The report includes ordered attempts and one terminal result: restored, targeted replacement, or environment blocked. |
...
| 7 | Prevent stale-agent late output from corrupting the route. | Add `role_binding_generation` and per-role `role_binding_epoch` bookkeeping. Late output from an older generation/epoch is quarantined and cannot complete current packets or gates. | Old-generation output creates a quarantine/control record, not route progress. |
...
| R5 | One failed role triggers broad recycle too early. | Unaffected agents lose context unnecessarily. | Hazard: `broad_recycle_without_targeted_attempt`. Invariant: broad recovery requires failed restore plus failed targeted replacement/slot reconciliation. | Test: one-role failure expects targeted scope first. |
| R6 | Capacity/full-slot conflict deadlocks because old role cannot close and new role cannot be opened. | Controller gets stuck with no usable role and no escalation. | Hazard: `capacity_full_without_escalation`. Invariant: close failure plus replacement capacity failure escalates to environment/user block. | Recovery report validation requires explicit blocker evidence in this case. |
| R7 | Full role binding recycle failure is hidden as success. | System appears resumed while no reliable role binding exists. | Hazard: `failed_full_recycle_marked_ready`. Invariant: failed full recycle blocks environment/user action. | Runtime validation rejects ready report without six usable roles. |
...
- recovery scope: all-six sweep, targeted role, slot reconciliation, full role-binding set;
...
- there is at least one successful full role binding recycle path;
