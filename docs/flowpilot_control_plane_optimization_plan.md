...
| 5 | Role-work and PM-issued packet result recipient normalization | Result content can be valid but rejected or misrouted because stale wording points PM-issued results to reviewer instead of PM. | Runtime/router treats PM role-work, material, research, and current-node PM-issued results as PM-returning before formal Reviewer gates, while preserving explicitly Reviewer-bound result delivery. | `packet_runtime.py`, `flowpilot_router.py`, contracts/card text | PM-issued results return to PM disposition first; Reviewer reviews only the PM formal package unless the result contract explicitly names Reviewer as the recipient. |
| 6 | Model-miss report completeness | PM can need a second FlowGuard operator request because the first model-miss report lacks self-check or PM decision fields. | Model-miss/FlowGuard operator request contracts require a small complete decision-support matrix on first return. | `runtime_kit` cards/contracts, `role_output_runtime.py` if needed | Adds fixed fields, not a new review loop. |
...
| R4 | Stop cleanup marks the run complete or leaves heartbeat/runtime roles/packet/frontier authority active. | Fails if stopped state still has active authorities or claims completion. |
| R5 | Recipient normalization accidentally bypasses PM disposition or reroutes explicitly Reviewer-bound results. | Fails if a PM-issued current-node/material/research/role-work result reaches Reviewer before PM disposition, or if an explicitly Reviewer-bound result no longer routes to Reviewer. |
...
| R7 | Model-miss report completeness adds a second mandatory loop instead of replacing a second loop. | Fails if model-miss cannot reach PM decision from one complete FlowGuard operator report. |
