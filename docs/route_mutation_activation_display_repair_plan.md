...
| 4 | Require route recheck before activation | route-scope FlowGuard, product FlowGuard, and reviewer route gates must pass against the candidate route before PM activation | Existing route-check gates plus new model invariant: activation before process recheck fails |
