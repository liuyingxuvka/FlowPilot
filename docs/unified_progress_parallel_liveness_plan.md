...
- heartbeat/manual-resume recovery that checks runtime-required role bindings one by one
...
controller-visible chat and progress files. Required-role liveness may run in
parallel, but PM resume decisions still wait for the final required role records
and the role rehydration report.
...
| 5 | Probe resume liveness as one batch. | Heartbeat/manual resume starts required role liveness checks before waiting for individual results. | The rehydration action contract requires concurrent batch evidence for runtime-required roles. |
| 6 | Preserve the resume join. | PM resume remains blocked until required role records are restored or replaced, current-run memory is injected, and the role rehydration report is written. | Parallel liveness reduces wait time but does not weaken the required-role gate. |
...
2. Upgrade the resume model for concurrent required-role liveness batch evidence.
