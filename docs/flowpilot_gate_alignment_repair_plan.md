...
| 2 | Separate unsupported historical/general events from gate completion | Keep unsupported historical events registered for old artifacts, but remove them from current expected gate waits and terminal gate completion checks. | `product_officer_model_report` can be recorded only as unsupported historical metadata and cannot close `product_architecture_modelability`. |
