...
This plan repairs the FlowPilot gate-alignment failure where a unsupported historical/general
...
| --- | --- | --- | --- |
| 1 | Declare gate contracts | Add machine-readable gate contract metadata for gate-bearing system cards and waits. | Delivered card envelopes and wait actions expose the active gate id, pass events, block events, required flag, and unsupported historical non-completion events. |
| 2 | Separate unsupported historical/general events from gate completion | Keep unsupported historical events registered for old artifacts, but remove them from current expected gate waits and terminal gate completion checks. | `flowguard_operator_model_report` can be recorded only as unsupported historical metadata and cannot close `product_architecture_modelability`. |
...
| 2 | A unsupported historical/general report is accepted as if it completed the gate. | `unsupported historical_event_accepted_without_required_gate_flag` |
...
| 7 | A production change reintroduces no-legal-next-action after a unsupported historical event. | dynamic return-path live projection historical/current findings |
...
3. Filter expected gate waits so unsupported historical events remain registered but are not
