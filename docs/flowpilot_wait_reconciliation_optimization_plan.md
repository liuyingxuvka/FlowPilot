...
| 4 | Active-holder fast lane expansion | Issue active-holder leases for material scan, research, and PM role-work packets when a live holder is known; keep fallback behavior for no live agent. | `flowpilot_router.py`, `packet_runtime.py`, worker/FlowGuard operator cards | Router-loop model and runtime tests prove direct ACK/progress/result works outside current-node packets without authority drift. |
...
| R8 | Invented or stale external event accepted | Role uses an event not currently authorized by Router. | Event-contract and prompt-coverage models must fail unlisted events and unsupported historical direct events outside `allowed_external_events`. |
