...
| 6 | Tests and unsupported historical | Tests expect officer cards in the default path. | Tests prove Reviewer remains required and officer cards are not emitted by default. Unsupported historical officer events may remain accepted for old runs but cannot block new runs. | `tests/test_flowpilot_router_runtime.py`, simulation runners |
...
| R12 | Existing runs already mid-officer-gate break unexpectedly. | Reviewer-only model fails if unsupported historical officer event unsupported historical is removed; runtime changes preserve unsupported historical event handlers and only change default next-card sequencing. |
