...
| 5 | Route activation previously required a Product Officer route check artifact. | Route activation requires the Product Officer product behavior model, the Process Officer process route model, and Reviewer route challenge pass. | `skills/flowpilot/assets/flowpilot_router.py`, router runtime tests |
| 6 | Reviewer route challenge and block paths previously required a Product Officer route check artifact as source evidence. | Reviewer route challenge uses the route draft, process route model, PM process-model decision, root contract, child-skill manifest, and original product behavior model evidence. | Reviewer card, router source-path specs, tests |
...
| 5 | Unsupported Product Officer route events accidentally become default blockers. | System card sequence no longer delivers the Product Officer route card by default; unsupported direct events are rejected by current event intake. |
| 6 | Reviewer report still requires deleted Product Officer route-check source evidence. | Router checked paths and reviewer card text are updated; runtime tests cover Reviewer pass without a second Product Officer route check. |
| 7 | Final closure still claims officer role slices are required even though no fresh final officer check runs. | Current final-ledger and terminal replay checks own final closure without barrier-equivalence layers. |
