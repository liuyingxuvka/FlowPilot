...
| --- | --- | --- | --- |
| O1 | Planning-phase route rewrite | If the root route is not executable before any child work starts, rewrite the route draft instead of creating a repair node. | route-scope FlowGuard, plus Reviewer |
| O2 | Normal node expansion | If planning discovers missing work, PM may add ordinary sibling or child nodes, but each node must have owner, input, output, evidence, and acceptance fields. | route-scope FlowGuard, plus Reviewer |
| O3 | Capability expansion | If a route or node adds product capability, product-scope FlowGuard must run before route-scope FlowGuard checks the changed route. | product-scope FlowGuard, then route-scope FlowGuard, plus Reviewer |
| O4 | Node-entry replanning | Before entering a parent/module/leaf, if the node lacks capability or is too coarse, PM may split or add ordinary nodes. This is not repair. | product-scope FlowGuard if product capability changes; route-scope FlowGuard for structure; Reviewer |
| O5 | In-progress replanning | If a current node is underway but has not produced a reviewed failed result, PM may revise the node internals or add same-scope nodes. This is still replanning, not repair. | Product/route-scope FlowGuard according to changed surface, plus Reviewer |
| O6 | Review-failure repair | Repair nodes are allowed only after a concrete reviewed failure or route-invalidating review finding. | Complete repair metadata, stale-evidence handling, route-scope FlowGuard, Reviewer recheck |
| O7 | Executable active node gate | A node cannot become active unless FlowPilot can tell whether it is parent/module/leaf/repair and how it will be executed or decomposed. | route-scope FlowGuard |
...
| B4 | Product capability changes without product-scope FlowGuard | Process may be valid while product behavior is wrong. | `capability_change_without_product_check` |
| B5 | route-scope FlowGuard runs before product-scope FlowGuard after capability expansion | Process would simulate the wrong product target. | `process_before_product_for_capability_change` |
| B6 | Route structure changes without route-scope FlowGuard | The route can become unreachable or non-executable. | `structure_change_without_process_check` |
...
3. Every accepted change has the right product-scope FlowGuard, route-scope FlowGuard,
