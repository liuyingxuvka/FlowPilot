...
| 3 | Require workers to use the bound child skill only for the current node slice and return `Child Skill Use Evidence`. | `cards/roles/worker.md`, `cards/roles/worker.md`, `templates/flowpilot/packets/result_body.template.md`, `contracts/contract_index.json` | Output-contract tests require evidence when active bindings exist. |
