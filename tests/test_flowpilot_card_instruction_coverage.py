...
class FlowPilotCardInstructionCoverageTests(unittest.TestCase):
    ...
    def test_repair_loop_threshold_routes_to_break_glass_guidance(self) -> None:
        ...
        self.assertIn("same repair lineage has repeated the same blocker problem five or more consecutive times", pm_repair)
        ...
        self.assertIn("same repair lineage has repeated the same blocker problem five or more consecutive times", controller)
        self.assertIn("similar blocker classes spread across different route nodes are ordinary repair evidence", controller)
        self.assertIn("same repair lineage has repeated the same blocker problem five or more consecutive times", break_glass)
        self.assertIn("false alarm", break_glass)
        self.assertIn("mechanical progress from semantic repair progress", flowguard_process)
        self.assertIn("five or more consecutive same-lineage attempts", flowguard_process)
