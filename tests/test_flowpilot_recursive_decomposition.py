...
class FlowPilotRecursiveDecompositionTests(unittest.TestCase):
    ...
    def test_runtime_prompts_and_templates_expose_recursive_decomposition_contracts(self) -> None:
        ...
        self.assertIn("canonical executable route tree", route_card)
        ...
        self.assertIn("Reviewer is the semantic decomposition quality gate", route_review_card)
        self.assertIn("parent/module node", current_node_card)
        self.assertIn("leaf_readiness_gate", node_plan_card)
        self.assertIn("parent backward review", " ".join(process_card.split()))
        self.assertIn("worker-decision leakage", process_card)
