...
<p align="center">
  <strong>A router-driven project-control layer for long AI-agent software work.</strong><br />
...
<p align="center">
  Source version: <strong>v0.9.25</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
<p align="center">
  <img src="./assets/readme-screenshots/startup-intake.png" alt="FlowPilot native startup intake UI with a work request field and toggles for runtime role assistance, scheduled continuation, and Cockpit UI" width="760" />
</p>
...
`v0.9.25` adds a generated FlowGuard project topology map for mature modeled systems. The map gives agents a first-pass view of model runners, model-test alignment families, code surfaces, test commands, evidence summaries, and known-bad/risk signals before non-trivial work, while keeping executable FlowGuard checks and tests as the actual evidence gates.
...
| Role authority | Separates Project Manager, Human-like Reviewer, FlowGuard operator, Worker roles, and Controller duties. | Makes approval and challenge visible instead of blending them into one prompt. |
...
1. Open a fresh AI CLI/window that supports local tools and, ideally, runtime role assistance and heartbeat/scheduled continuation.
...
`v0.9.25` 新增生成式 FlowGuard 项目拓扑图，面向已经有大量模型的成熟系统。这个地图会在非平凡工作前给 agent 一个第一眼结构视图：模型 runner、model-test alignment family、代码面、测试命令、证据摘要和 known-bad/risk 信号；但真正的证据 gate 仍然必须来自可执行的 FlowGuard 检查和测试。
...
| Role authority | 分离 PM、Human-like Reviewer、FlowGuard operator、FlowGuard operator、Worker 和 Controller 职责。 | 让批准和挑战可见，而不是混进同一个 prompt。 |
...
1. 打开一个支持本地工具的全新 AI CLI/window，最好支持 runtime role assistance 和 heartbeat/scheduled continuation。
