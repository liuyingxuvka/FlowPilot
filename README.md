...
- Current source version in this checkout: **v0.9.6**.
...
- v0.9.6 has had a behavior-preserving maintenance pass: the public protocol,
  JSON shapes, and invocation guidance stay the same, while runtime/model/check
  internals are easier to inspect and validate.
...
- v0.9.6 已做过一轮行为不变的维护：公开协议、JSON 形状和调用说明保持不变，
  但 runtime、模型和检查脚本内部更容易检查和验证。
