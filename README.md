...
| Practical failure | FlowPilot mechanism |
| --- | --- |
| Prompt contamination | PM, reviewer, officers, workers, and Controller receive role-scoped cards instead of one shared prompt surface |
| Long-horizon drift | The active route, frontier, current node, waits, packets, and ledgers are file-backed and router-owned |
| Lost or stale instructions | System-card delivery and acknowledgement make role instructions explicit, replayable, and separate from semantic work completion |
| Authority collapse | The Controller can relay and report metadata, but sealed bodies, worker execution, review, and approval stay with their roles |
| Stale evidence | Route mutation, peer writes, failed reviews, and later changes can invalidate earlier evidence instead of leaving old green status |
| Resume ambiguity | Heartbeat/manual resume re-enters through current route state and pending waits rather than guessing from chat history |
| Child-skill misuse | PM selection, gate manifests, reviewer challenge, and FlowGuard checks turn child-skill use into route evidence |
| Premature completion | Final closure waits for terminal replay and a route-wide ledger, not just a successful local edit |
...
| 真实失败方式 | FlowPilot 机制 |
| --- | --- |
| prompt 互相污染 | PM、reviewer、officer、worker、Controller 使用 role-scoped cards，而不是共享一整块 prompt surface |
| 长距离 drift | active route、frontier、current node、waits、packets、ledgers 都是 file-backed 且 router-owned |
| 指令丢失或过期 | system-card delivery 和 ACK 把角色指令变成显式、可重放、且与语义工作完成分离的证据 |
| authority collapse | Controller 只能 relay 和报告 metadata；sealed body、worker execution、review、approval 仍属于各自 role |
| stale evidence | route mutation、peer write、failed review 和后续变化可以让旧证据失效，而不是保留旧 green status |
| resume 模糊 | heartbeat/manual resume 从当前 route state 和 pending waits 回来，而不是从 chat history 猜 |
| child-skill 滥用 | PM selection、gate manifest、reviewer challenge 和 FlowGuard check 让 child-skill 使用变成 route evidence |
| 过早完成 | final closure 需要 terminal replay 和 route-wide ledger，而不是一个局部 edit 成功 |
