...
<p align="center">
  Source version: <strong>v0.9.24</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
`v0.9.24` focuses on the new formal runtime lifecycle guard. Nonterminal runs keep `controller_stop_allowed: false`, manual resume and patrol rehydrate from the current-run ledger, stale or late results are quarantined, and terminal return is authorized only after final closure.
...
`v0.9.24` 主要强化新版 formal runtime 的 lifecycle guard：未完成时保持 `controller_stop_allowed: false`，manual resume 和 patrol 从 current-run ledger 重建状态，过期或迟到结果会被 quarantine，只有 final closure 完成后才允许 terminal return。
