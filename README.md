...
- v0.9.7 has had a behavior-preserving structure simplification pass: the public protocol,
  JSON shapes, and invocation guidance stay the same, while runtime/model/check
  internals and domain test entrypoints are easier to inspect and validate.
...
- v0.9.7 已做过一轮行为不变的结构简化：公开协议、JSON 形状和调用说明保持不变，
  但 runtime、模型、检查脚本和测试域入口更容易检查和验证。
