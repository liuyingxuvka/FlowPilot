...
<p align="center">
  Source version: <strong>v0.10.0</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
`v0.10.0` completes the new FlowPilot topology language pass: current prompts, templates, runtime cards, models, and tests now use the single `flowguard_operator` responsibility, generic requested worker responsibilities, mandatory node pre-work FlowGuard gates, PM-authored node context packages, and current-effective packet outcome authority.
...
`v0.10.0` 完成新版 FlowPilot 拓扑语言清理：当前 prompts、templates、runtime cards、models 和 tests 现在统一使用单一 `flowguard_operator` responsibility、按需 worker responsibility、强制 node pre-work FlowGuard gate、PM-authored node context package，以及 current-effective packet outcome authority。
