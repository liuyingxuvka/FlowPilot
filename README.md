...
<p align="center">
  Source version: <strong>v0.9.10</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
- Current source version in this checkout: **v0.9.10**.
...
- v0.9.10 has had behavior-preserving StructureMesh-guided structure
  maintenance: the public
  protocol, JSON shapes, and invocation guidance stay the same, while
  the router skeleton and the largest startup, protocol, packet, event, route,
  scheduler, repair, card, and action-handler owner bands moved into focused
  modules. The latest maintenance gate adds
  model-derived StructureMesh target structures, TestMesh target split
  derivations, FlowGuard Model-Test Alignment, and background validation
  artifact boundaries.
...
- v0.9.10 已做过基于 StructureMesh 的行为不变结构维护：公开协议、JSON 形状和调用
  说明保持不变，router skeleton 以及最大的 startup、protocol、packet、event、route、
  scheduler、repair、card 和 action-handler owner 大块已经移入聚焦模块。最新维护 gate 同时加入模型导出的 StructureMesh 目标结构、TestMesh 目标测试
  拆分、FlowGuard Model-Test Alignment 和后台证据边界。
