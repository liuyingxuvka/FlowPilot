...
| --- | --- | --- |
| Project Manager (PM) | Preserve user intent, select relevant capabilities, design the route, issue work packets, integrate results, decide repair/replan/continue/stop | Execute worker packets or approve its own output as final evidence |
| Worker | Plan and execute a complete bounded workstream with the allowed files, tools, child skills, delegation, verification, repair, and expected output shape | Redesign the route, mark the node complete, or bypass review |
...
Every substantive PM, Worker, research Worker, Reviewer, and FlowGuard Operator assignment includes a numbered local plan and a returned per-step completion/evidence report. Reviewer compares that report with the real artifact and current evidence; Controller is excluded and follows only the Runtime foreground ledger. Runtime also performs one mandatory shallow local skill/capability availability scan, while PM selects which candidates deserve deep reading. Reading and research are ordinary work packages—there is no mandatory material-scan form or universal material-sufficiency gate.
...
-> shallow local skill/capability inventory and PM selection
-> PM source/evidence understanding through ordinary work when needed
...
FlowPilot's SkillGuard integration is declarative and native-integrated. The
current V2 contract trio is checked with
`python scripts\refresh_flowpilot_skillguard_contract.py --check`; FlowPilot
still owns execution, while SkillGuard verifies contract parity and consumes
current receipts. Native integration binds the exact four FlowPilot routes and
all seven declared checks to their existing source owners; missing, extra,
duplicate, defaulted, or fallback bindings block global selection. A
contract-depth pass is not an execution-depth claim.
...
| 角色 | 负责什么 | 不能默默做什么 |
| --- | --- | --- |
| Project Manager (PM) | 保留用户意图、选择相关能力、设计 route、发 work packet、整合结果、决定 repair/replan/continue/stop | 自己执行 worker packet，或把自己的输出当最终证据批准 |
| Worker | 在允许的文件、工具和 child skill 内规划并执行完整的有边界 workstream，包括委派整合、验证、修复和报告 | 重新设计 route、标记 node 完成，或绕过 review |
...
每个实质性的 PM、Worker、research Worker、Reviewer 和 FlowGuard Operator 任务都要先写编号本地计划，并在现有结果里逐步报告完成状态和证据。Reviewer 会把报告与真实产物和当前证据对照；Controller 不参与实质计划，只遵循 Runtime 前台账本。Runtime 仍强制做一次本机技能/能力可用性的浅扫描，PM 再选择哪些候选值得深读。阅读和研究属于普通工作包，不再有强制材料扫描表或通用材料充分性门。
...
FlowPilot 的 SkillGuard 集成是声明式、原生集成的。当前 V2 合同三件套使用
`python scripts\refresh_flowpilot_skillguard_contract.py --check` 检查；实际执行
仍由 FlowPilot 唯一所有，SkillGuard 只检查合同一致性并消费当前收据。原生集成
会把现有四条 FlowPilot 路由和全部七项已声明检查精确绑定到其现有源码所有者；
缺失、多余、重复、默认补全或 fallback 绑定都会阻断全局选择。合同深度通过
不等于已经证明 AI 实际执行深度。
