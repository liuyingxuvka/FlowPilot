...
`v0.10.13` hardens fake-AI rehearsal and reviewer-window coverage on top of the `v0.10.12` repair-lineage release. Planning packets surface active acceptance-owner coverage before first response, node-acceptance packets expose acceptance-item projection rows, malformed AI result bodies are exercised by the contract-driven fake AI responder, reviewer packets carry a runtime-checkable review window, and the fifth same-lineage blocker routes to BreakGlass consistently across runtime and role cards.
...
`v0.10.13` 在 `v0.10.12` repair-lineage release 上强化 fake-AI rehearsal 和 reviewer-window coverage。Planning packets 会在 first response 前暴露 active acceptance-owner coverage；node-acceptance packets 会暴露 acceptance-item projection rows；contract-driven fake AI responder 会覆盖 malformed AI result bodies；reviewer packets 带 runtime-checkable review window；第五次 same-lineage blocker 会在 runtime 和 role cards 中一致路由到 BreakGlass。
