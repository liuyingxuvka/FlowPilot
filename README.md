...
<p align="center">
  Source version: <strong>v0.9.13</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
`v0.9.13` focuses on persistent Router daemon recovery and user-stop handling. Dead-owner JSON write locks are taken over with diagnostics, live writers defer instead of killing the daemon, and terminal runs fence daemon, startup, heartbeat, and status projections early.
...
`v0.9.13` 主要强化 persistent Router daemon recovery 和 user-stop handling：dead-owner JSON write lock 会带诊断接管，live writer 会 defer 而不是杀掉 daemon，terminal run 会更早 fence daemon、startup、heartbeat 和 status projection。
