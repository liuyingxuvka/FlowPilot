...
<p align="center">
  Source version: <strong>v0.9.8</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
- Current source version in this checkout: **v0.9.8**.
...
- v0.9.8 has had behavior-preserving StructureMesh-guided structure
  maintenance: the public
  protocol, JSON shapes, and invocation guidance stay the same, while
  the router protocol/catalog ownership moved into a focused module. The
  latest maintenance gate adds model-derived StructureMesh target structures,
  TestMesh target split derivations, FlowGuard Model-Test Alignment, and
...
- v0.9.8 已做过基于 StructureMesh 的行为不变结构维护：公开协议、JSON 形状和调用
  说明保持不变，router 的 protocol/catalog 归属已经移入独立模块。最新维护 gate 同时
  加入模型导出的 StructureMesh 目标结构、TestMesh 目标测试拆分、FlowGuard
  Model-Test Alignment 和后台证据边界。
