...
<p align="center">
  Source version: <strong>v0.9.7</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
- Current source version in this checkout: **v0.9.7**.
...
- v0.9.7 has had behavior-preserving structure convergence passes: the public
  protocol, JSON shapes, and invocation guidance stay the same, while
  runtime/model/check internals and domain test entrypoints are easier to
  inspect and validate. The latest maintenance gate adds StructureMesh/TestMesh
  evidence plus FlowGuard Model-Test Alignment for router split ownership,
  child-model facade ownership, background validation artifacts, and ordinary
...
- v0.9.7 已做过行为不变的结构收敛维护：公开协议、JSON 形状和调用说明保持不变，
  但 runtime、模型、检查脚本和测试域入口更容易检查和验证。最新维护 gate 同时加入
  StructureMesh/TestMesh 和 FlowGuard Model-Test Alignment，用来说明拆分归属、后台证据
  和普通测试覆盖边界。
