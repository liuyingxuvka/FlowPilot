...
<p align="center">
  Source version: <strong>v0.9.9</strong> · MIT License · Codex skill source package
</p>
<!-- README HERO END -->
...
- Current source version in this checkout: **v0.9.9**.
...
- v0.9.9 has had behavior-preserving StructureMesh-guided structure
  maintenance: the public
  protocol, JSON shapes, and invocation guidance stay the same, while
  the router protocol/catalog ownership and eight additional router behavior
  owner bands moved into focused modules. The latest maintenance gate adds
...
- v0.9.9 已做过基于 StructureMesh 的行为不变结构维护：公开协议、JSON 形状和调用
  说明保持不变，router 的 protocol/catalog 和另外八个 router 行为 owner 大块已经移入
  独立模块。最新维护 gate 同时加入模型导出的 StructureMesh 目标结构、TestMesh 目标测试
