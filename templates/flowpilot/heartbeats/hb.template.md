...
3. Audit the mail chain before doing normal continuation: every formal
...
4. Ask PM for `PM_DECISION` from the current frontier. PM must include
...
5. If PM issues a packet envelope/body pair, sign and relay only the envelope to
   the addressed role. The recipient must verify the controller signature before
   opening the body. Include `ROLE_REMINDER`.
6. If a worker already has an unfinished packet, resume that exact packet only
...
7. If a worker result exists, sign and route only the `RESULT_ENVELOPE` to
...
8. Continue the internal packet loop only when PM says `stop_for_user: false`;
   otherwise write the controlled stop notice.
9. If the holder, worker identity, prior reviewer dispatch, relay signature,
