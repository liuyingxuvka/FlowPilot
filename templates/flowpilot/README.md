...
18. Probe host continuation capability. If real wakeups are supported, create
   the automated bundle: heartbeat, paired watchdog, and quiet singleton global
   supervisor evidence. Do not auto-create high-frequency global cron; record
   setup-required/on-demand evidence when no quiet singleton exists. If
   unsupported, record `manual-resume` and do not create any of those
   automations.
...
20. Before terminal completion, have the project manager rebuild
   ...
   collect all effective node gates and child-skill gates, check stale evidence
   and superseded-node explanations, get human-like backward review, and allow
   PM completion approval only when unresolved count is zero.
21. Update heartbeats, role memory packets, node reports, and checkpoints
   after verified progress.

## Files

- `state.template.json`: current pointer and host continuation mode.
- `execution_frontier.template.json`: current route version, active node, next
  ...
  native/fallback visible plan sync method, and visible plan projection depth.
...
- `heartbeats/hb.template.json`: heartbeat shell.
- `watchdog/watchdog.template.json`: external stale-heartbeat check shell.
- `diagrams/current-route-map.template.mmd`: Mermaid route-map source for chat
  fallback and UI display.
- `diagrams/current-route-map.template.md`: Markdown preview wrapper for the
  same Mermaid route map.
