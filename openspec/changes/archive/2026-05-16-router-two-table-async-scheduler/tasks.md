...
- [x] 2.1 Add a Router scheduler ledger with row ids, barrier classification, scope metadata, receipt state, and reconciliation state.
- [x] 2.2 Keep Controller action ledger rows simple while linking them to Router scheduler rows.
- [x] 2.3 Add deterministic daemon idempotency keys so retries do not duplicate queued Controller work.
- [x] 2.4 Teach daemon ticks to reconcile receipts, apply Router-visible postconditions, and enqueue independent rows until a barrier.
- [x] 2.5 Unify startup pre-review cleanup with the current-scope reconciliation action using `scope_kind=startup`.
- [x] 2.6 Preserve PM startup activation's existing same-role ACK blocker without adding a second activation join.
...
- [x] 3.1 Add runtime tests for multi-row daemon enqueueing and the two-table split.
- [x] 3.2 Add runtime tests for startup Reviewer gating through current-scope reconciliation.
- [x] 3.3 Add runtime tests for stateful Controller receipt postcondition reconciliation, including controller boundary confirmation.
...
- [x] 4.2 Review peer-agent changes and include compatible work in final staging.
- [x] 4.3 Record FlowGuard adoption and KB postflight notes.
- [x] 4.4 Commit the synchronized local git version.
