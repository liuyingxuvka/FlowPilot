...
- **WHEN** the Router daemon is live, no ordinary Controller action is ready,
  and Controller runs the patrol timer command
- **THEN** the command returns `patrol_result=continue_patrol`, includes the
  anti-exit purpose, names the next command, and instructs Controller to rerun
  that command and wait for its next output

#### Scenario: New Controller work wakes patrol
- **WHEN** a ready Controller action exists while Controller runs the patrol
  timer command
- **THEN** the command returns `patrol_result=new_controller_work` and instructs
  Controller to read the Controller action ledger and process ready rows from
  top to bottom

#### Scenario: Terminal state permits exit
- **WHEN** the monitored run is terminal and `controller_stop_allowed` is true
- **THEN** the command returns `patrol_result=terminal_return` and allows the
  foreground Controller to end
