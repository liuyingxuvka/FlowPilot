...
FlowPilot SHALL replay valid current-run rehydration receipts and crew
rehydration reports into the existing `resume_roles_restored` postcondition
before materializing a control blocker for missing resume role restoration.

#### Scenario: Valid six-role report satisfies postcondition
- **WHEN** Router evaluates resume after heartbeat or manual wake
- **AND** a current-run crew rehydration report shows all six roles ready
...
- **WHEN** the current-run rehydration evidence is missing a role, stale,
