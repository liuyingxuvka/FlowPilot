...
- [x] 1.2 Add requirements for symmetric packet lifecycle across PM, FlowGuard, Reviewer, Validation, and Closure.
...
- [x] 2.2 Make `submit-result` advance task -> FlowGuard -> review -> validation -> closure packets.
...
- [x] 3.1 Add tests that reproduce the live-run FlowGuard operator lease failure before repair.
- [x] 3.2 Add tests that reviewer status projection has packet id and ACK before review result and no active dirty lease after closure.
