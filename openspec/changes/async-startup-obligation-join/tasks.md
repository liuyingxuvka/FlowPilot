...
- [x] 1.2 Update the startup FlowGuard model so startup ACK join uses the common ledger path.
...
- [ ] 2.2 Enforce startup activation join through the existing pending-card-return dependency blocker.
- [ ] 2.3 Preserve the same Controller action ledger and card ACK ledgers; do not add a startup-only table.

## 3. Tests And Validation

- [ ] 3.1 Add focused router tests for startup card delivery continuing while startup ACKs are pending.
- [ ] 3.2 Add focused router tests for PM startup activation blocked until startup ACK join is clean.
