...
of expanding the full parent state graph.
...
- **THEN** the runner MUST complete by checking thin parent evidence and MUST
  report that the result is not a full graph regression
...
### Requirement: Full parent regressions remain available
...
shared kernel, parent-only thin check, full-regression-only obligation, or explicit
out-of-scope reason.
...
#### Scenario: Full-regression-only family remains
- **WHEN** a parent invariant family cannot yet be represented by child or thin
  parent evidence
- **THEN** the ledger MUST mark that family as full-regression-only and the thin
...
confidence that requires current full regression evidence or an explicit
approved equivalence gate.

#### Scenario: Thin checks pass without full evidence
- **WHEN** thin Meta and Capability checks pass but full parent
...
- **WHEN** thin checks pass and current full regression proof or
