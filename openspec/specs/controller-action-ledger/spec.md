# controller-action-ledger Specification
...
- **WHEN** Controller writes a `done` receipt for an active pending action whose
  Router postconditions are already complete or not stateful
- **THEN** Router MUST mark the action done in the ledger
- **AND** Router MUST clear the active pending action before computing the next
  action

#### Scenario: Receipt marks active stateful action done
- **WHEN** Controller writes a `done` receipt for an action that requires
  Router-owned stateful postconditions
- **THEN** Router MUST apply those postconditions from a complete receipt
  payload or produce a repair blocker
- **AND** Router MUST NOT rely on the receipt metadata alone as proof that the
  postconditions were written
