...
### Requirement: Unsupported historical project runners emit stderr progress
...
- **WHEN** a unsupported historical runner builds its reachable graph
...
Unsupported historical project runner progress SHALL be disabled when `FLOWGUARD_PROGRESS=0`.

#### Scenario: Strict environment disables progress
- **WHEN** `FLOWGUARD_PROGRESS=0` is set
- **THEN** the unsupported historical runner suppresses progress messages while preserving check behavior
