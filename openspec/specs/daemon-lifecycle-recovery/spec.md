...
## Requirements
### Requirement: Manual resume supervises daemon lifecycle
FlowPilot manual resume and daemon-liveness recovery SHALL supervise Router
daemon, Controller executor, and current role-binding liveness instead of
acting as the normal way to advance route work.

#### Scenario: Router daemon is alive
- **WHEN** manual resume or daemon-liveness recovery wakes and finds a live
  Router daemon for the current run
...
- **WHEN** manual resume or daemon-liveness recovery wakes and finds no live
  Router daemon or a stale daemon lock
...
Manual resume and daemon-liveness recovery SHALL verify that a Controller
executor is attached to the current run's Controller action ledger.

#### Scenario: Controller executor is attached
- **WHEN** manual resume or daemon-liveness recovery confirms Controller is
  attached to the active run ledger
...
- **WHEN** manual resume or daemon-liveness recovery cannot verify a live
  Controller executor attachment
...
### Requirement: Role-binding recovery remains current-run scoped
Manual resume and daemon-liveness recovery SHALL restore or replace current
runtime-required background role bindings only from current-run role memory and
current-run role binding records.

#### Scenario: Role is live
- **WHEN** a role binding is live, current-run scoped, and addressable
- **THEN** recovery MUST preserve that role binding rather than replacing it

#### Scenario: Role is missing or stale
- **WHEN** a role binding is missing, cancelled, stale, unknown, or not
  addressable
- **THEN** recovery MUST open or rehydrate a replacement role binding from
  current-run memory before work that depends on that role continues

### Requirement: Terminal cleanup stops daemon mode
Terminal lifecycle events SHALL stop Router daemon mode, Controller ledger
execution, foreground patrol, and role activity for the run.
...
- **THEN** the run MUST remain terminal and manual resume or daemon-liveness
  recovery MUST NOT restart Router daemon or rehydrate roles unless the user
  explicitly starts a new formal FlowPilot run

### Requirement: Recovery never trusts chat history as route state
Manual resume and daemon-liveness recovery SHALL recover only from current-run
persisted state, daemon status, action ledgers, mailbox evidence, role-binding
memory, and Router records.
...
- **THEN** manual resume, daemon-liveness recovery, and Router daemon recovery
  MUST still treat the run as terminal
- **AND** they MUST NOT restart daemon mode, rehydrate roles, create
  continuation automations, or continue route work for that run.
...
- **AND** Router MUST NOT expose unrelated startup, foreground patrol, role, or
  route actions for the same run.
...
current-run pointers, daemon status, manual-resume evidence,
...
- **THEN** FlowPilot MUST mark the run lifecycle as stopped or terminal, stop
  daemon active ticking, suppress manual resume restart, supersede or
...
- **WHEN** manual resume or daemon-liveness recovery observes a stopped run
