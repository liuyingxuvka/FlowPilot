...
def _next_pm_role_work_request_action(router: ModuleType, project_root: Path, run_state: dict[str, Any], run_root: Path) -> dict[str, Any] | None:
    ...
    if batch_records:
        ...
        lifecycle_index_path = router._flowguard_operator_request_lifecycle_index_path(run_root)
        ...
        if all((record.get('status') == 'result_relayed_to_pm' for record in batch_records)):
            action = _expected_role_decision_wait_action(project_root, run_state, run_root, label='controller_waits_for_pm_role_work_batch_result_decision', summary='The full role-work result batch reached PM and Controller must wait for one PM batch disposition.', allowed_external_events=[PM_ROLE_WORK_RESULT_DECISION_EVENT], to_role='project_manager', payload_contract=router._pm_role_work_result_decision_payload_contract(name='pm_role_work_batch_result_decision', required_fields=['decided_by_role', 'batch_id', 'decision'], allowed_values={'decided_by_role': ['project_manager'], 'decision': sorted(PM_ROLE_WORK_TERMINAL_DECISIONS)}, records=batch_records, expected_batch_id=str(index.get('active_batch_id') or '')))
            if router._pm_role_work_records_are_nonblocking(batch_records):
                action['nonblocking_wait'] = True
                action['dependency_class'] = router._pm_role_work_records_dependency_class(batch_records)
            return action
