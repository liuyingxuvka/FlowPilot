"""External event record entrypoint split from flowpilot_router_event_dispatcher."""
...
def _record_external_event_unchecked(router: ModuleType, project_root: Path, event: str, payload: dict[str, Any] | None=None, *, envelope_path: str | None=None, envelope_hash: str | None=None) -> dict[str, Any]:
    ...
    if run_state['flags'].get(flag):
        if not payload_normalized_for_replay:
            ...
            payload_normalized_for_replay = True
            scoped_identity = _scoped_event_identity(project_root, run_root, run_state, event, payload)
        quarantined = _quarantine_direct_scoped_event_conflict(router, project_root, run_root, run_state, event=event, payload=payload, envelope_path=envelope_path, envelope_hash=envelope_hash, scoped_identity=scoped_identity)
        if quarantined is not None:
            return quarantined
        _check_scoped_event_conflict(run_state, scoped_identity)
        authority_split = _repair_direct_package_disposition_authority_split(router, project_root, run_root, run_state, event=event, payload=payload, scoped_identity=scoped_identity, envelope_hash=envelope_hash)
