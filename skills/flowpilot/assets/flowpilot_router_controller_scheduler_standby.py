...
from flowpilot_router_controller_scheduler_standby_task import (
...
def _build_foreground_controller_standby_snapshot(router: ModuleType, project_root: Path, run_root: Path, run_state: dict[str, Any], *, started_at: str, start_monotonic: float, poll_count: int, max_seconds: float, poll_seconds: float) -> dict[str, Any]:
    ...
    patrol_monitor = _router_daemon_patrol_monitor(lock, lock_liveness, status_exists=status_path.exists(), status_ok=status_ok)
    daemon_liveness_check_required = patrol_monitor.get('status') == 'check_liveness'
    ...
    while True:
        ...
        if elapsed >= max_seconds:
            ...
            snapshot['final_answer_preflight'] = {
                ...
                'status_projection_is_not_stop_authority': True,
                'authority_source': 'router_daemon_status_and_controller_action_ledger',
                'additional_authority_sources': ['wait_receipt_audit'],
...
def controller_patrol_timer(router: ModuleType, project_root: Path, *, seconds: float=_DEFAULT_SENTINEL) -> dict[str, Any]:
    ...
    if standby_state == 'controller_action_ready' or pending_ids:
        ...
        controller_instruction = str(router_daemon.get('controller_instruction') or 'Daemon patrol needs a Controller liveness check. If the daemon is alive, stay attached and continue. If it is dead, recover the current-run Router daemon without starting a second live writer.')
