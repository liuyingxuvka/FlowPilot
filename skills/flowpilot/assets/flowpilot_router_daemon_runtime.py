...
def _router_daemon_artifact_size_summary(router: ModuleType, project_root: Path, run_root: Path) -> dict[str, Any]:
    ...
    for name, path in key_paths.items():
        try:
            key_sizes[name] = {'path': router.project_relative(project_root, path), 'exists': path.exists(), 'size_bytes': path.stat().st_size if path.exists() and path.is_file() else 0}
    ...
    return {'key_files': key_sizes, 'runtime_file_count': file_count, 'runtime_total_bytes': total_bytes, 'largest_runtime_files': largest[:10]}


def _router_daemon_error_diagnostics(router: ModuleType, project_root: Path, run_root: Path, run_state: dict[str, Any], exc: BaseException) -> dict[str, Any]:
