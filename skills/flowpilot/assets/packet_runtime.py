...
def packet_identity_boundary(role: str) -> str:
    return (
        ...
        f"required_return: Packet ACK is receipt only; ACK is not completion. After ACK, execute this packet body, then write the result body authored as `{role}` only to the result body file. If Router issued an active-holder lease for this packet, acknowledge and submit the sealed result directly to Router through that lease; otherwise return only the runtime envelope metadata required by Router. Do not include result-body content in chat.\n"
