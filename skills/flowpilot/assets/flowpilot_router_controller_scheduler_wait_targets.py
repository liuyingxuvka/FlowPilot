...
def _wait_target_due_state(router: ModuleType, *, wait_class: str, elapsed_seconds: int | None, last_reminder_elapsed_seconds: int | None, evidence_exists: bool) -> dict[str, Any]:
    ...
    if wait_class == 'ack':
        ...
        blocker_after_seconds = WAIT_TARGET_ACK_REPLACE_SECONDS
        if not evidence_exists and elapsed_seconds is not None:
            ...
            blocker_required = elapsed_seconds >= WAIT_TARGET_ACK_REPLACE_SECONDS
            if blocker_required:
                blocker_reason = 'ack_missing_after_ten_minutes'
            if blocker_required:
                next_due_seconds = 0
                next_due_reason = 'ack_replacement'
            elif reminder_due:
                next_due_seconds = 0
                ...
                candidates.append((WAIT_TARGET_ACK_REPLACE_SECONDS - elapsed_seconds, 'ack_replacement'))
        ...
        if not evidence_exists and elapsed_seconds is not None and elapsed_seconds >= WAIT_TARGET_REPORT_REPLACE_SECONDS:
            reissue_required = True
            reissue_reason = 'role_no_fresh_progress_after_thirty_minutes'
        if blocker_required:
            next_due_seconds = 0
            next_due_reason = 'role_wait_blocker'
        elif reissue_required:
            next_due_seconds = 0
            next_due_reason = 'role_progress_reissue'
        elif reminder_due:
            next_due_seconds = 0
            next_due_reason = 'report_progress_reminder'
...
def _pending_wait_summary(router: ModuleType, run_state: dict[str, Any], *, project_root: Path | None=None) -> dict[str, Any]:
    ...
    due_state = router._wait_target_due_state(wait_class=wait_class, elapsed_seconds=elapsed_seconds, last_reminder_elapsed_seconds=last_reminder_elapsed_seconds, evidence_exists=evidence_exists)
    return {'action_type': pending.get('action_type'), 'label': pending.get('label'), 'to_role': pending.get('to_role'), 'waiting_for_role': pending.get('waiting_for_role') or pending.get('to_role'), 'allowed_external_events': pending.get('allowed_external_events') or [], 'expected_return_path': expected_return_path, 'controller_action_id': pending.get('controller_action_id'), 'resource_lifecycle': pending.get('resource_lifecycle'), 'artifact_committed': pending.get('artifact_committed'), 'relay_allowed': pending.get('relay_allowed'), 'wait_class': wait_class, 'target_role': target_role or None, 'wait_reason': wait_reason or None, 'started_at': started_at, 'elapsed_seconds': elapsed_seconds, 'expected_evidence': {'path': expected_return_path, 'exists': evidence_exists, 'controller_visible_only': True}, 'reminder': {'text': pending.get('wait_reminder_text') or router._wait_target_reminder_text(wait_class, target_role, wait_reason), 'last_sent_at': last_reminder_at, 'due': due_state['reminder_due'], 'interval_seconds': due_state['reminder_interval_seconds'], 'controller_must_use_router_authored_text': True}, 'ack_progress_evidence_policy': {'current_wait_authority': 'expected_return_path_or_ack_progress_evidence', 'unsupported_legacy_payloads_rejected': True}, 'controller_local_self_audit': {'required': wait_class == 'controller_local_action', 'reminder_allowed': False, 'check_action_ledger': True, 'check_receipts': True}, 'next_due': {'seconds': due_state['next_due_seconds'], 'reason': due_state['next_due_reason'], 'strict_wait_until_due': wait_class in {'ack', 'report_result', 'controller_local_action'}}, 'reissue': {'required': due_state['reissue_required'], 'reason': due_state['reissue_reason'], 'event': 'controller_reports_role_no_output' if due_state['reissue_required'] else None, 'record_event_payload': {'role_key': target_role, 'target_role_keys': [target_role] if target_role else [], 'wait_class': wait_class, 'wait_reason': wait_reason or None, 'expected_return_path': expected_return_path, 'elapsed_seconds': elapsed_seconds, 'current_controller_action_id': pending.get('controller_action_id'), 'router_scheduler_row_id': pending.get('router_scheduler_row_id')} if due_state['reissue_required'] else None, 'same_work_reissue_before_role_recovery': bool(due_state['reissue_required']), 'pm_recovery_required': False}, 'blocker': {'required': due_state['blocker_required'], 'reason': due_state['blocker_reason'], 'event': 'controller_reports_role_wait_blocker' if due_state['blocker_required'] else None, 'record_event_payload': {'role_key': target_role, 'target_role_keys': [target_role] if target_role else [], 'wait_class': wait_class, 'wait_reason': wait_reason or None, 'expected_return_path': expected_return_path, 'elapsed_seconds': elapsed_seconds} if due_state['blocker_required'] else None, 'pm_recovery_required': bool(due_state['blocker_required'])}}
...
def _wait_target_reminder_payload_contract(router: ModuleType) -> dict[str, Any]:
    _bind_router(router)
    return _payload_contract(name='wait_target_reminder_receipt', required_object='payload', required_fields=['target_role', 'delivered_to_role', 'reminder_text_sha256', 'sealed_body_reads'], optional_fields=['delivered_at', 'delivery_channel'], allowed_values={'sealed_body_reads': [False]}, structural_requirements=['reminder_text_sha256 must match the Router-authored reminder_text on the action row', 'Controller must send the reminder_text exactly as supplied and must not paste sealed result bodies', 'target_role and delivered_to_role must name the wait target role', 'Receipt payload must not include host-liveness, liveness-probe, bounded-wait, or timeout_unknown fields'], description="Receipt proving Controller sent the Router-authored wait reminder to the current waiting role. This records reminder delivery only; it does not satisfy the role's original ACK, progress, blocker, or result-return obligation.")
