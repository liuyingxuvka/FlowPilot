...
def _next_material_packet_action(router: ModuleType, project_root: Path, run_state: dict[str, Any], run_root: Path) -> dict[str, Any] | None:
    ...
    if (flags.get('pm_material_packets_issued') or generation_progress_active) and (not packets_relayed):
        ...
        role_binding_action = _open_current_role_agent_for_packet_plan(router, project_root, run_root, run_state, active_holder_plan, packet_family='material_scan', next_action_type='relay_material_scan_packets')
    ...
    if results_relayed_to_pm and (not pm_disposition_recorded):
        return _expected_role_decision_wait_action(project_root, run_state, run_root, label='controller_waits_for_pm_material_scan_result_disposition', summary='Material scan results reached PM and Controller must wait for PM to record a result disposition before any reviewer sufficiency gate.', allowed_external_events=['pm_records_material_scan_result_disposition'], to_role='project_manager', payload_contract=pm_package_result_disposition_payload_contract('pm_material_scan_result_disposition'))
    return None
