...
def _build_current_status_summary(router: ModuleType, run_root: Path, run_state: dict[str, Any], *, route_payload: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    daemon_patrol = daemon_status.get('daemon_patrol') if isinstance(daemon_status.get('daemon_patrol'), dict) else {}
    daemon_liveness_check_required = not daemon_status_ok or not daemon_lock_live or daemon_patrol.get('status') == 'check_liveness' or (daemon_lifecycle in {'daemon_error', 'daemon_stale_or_missing'})
    ...
    active_set_status = {'authority': active_ui_task_catalog.get('authority'), 'scope_kind': active_ui_task_catalog.get('scope_kind'), 'current_focus_target_id': (active_ui_task_catalog.get('operation_targets') or {}).get('current_focus'), 'active_count': len(active_ui_task_catalog.get('active_tasks') or []), 'background_active_count': len(active_ui_task_catalog.get('background_active_tasks') or []), 'stale_residue_count': len(active_ui_task_catalog.get('stale_residue_tasks') or []), 'global_main_required': bool(active_ui_task_catalog.get('global_main_required')), 'operation_target_required': bool(active_ui_task_catalog.get('operation_target_required')), 'plain_language_guidance': {'en': 'Parallel Flow blocks are allowed; actions must target one block or all active blocks.', 'zh': '允许多个 Flow 块并行；操作必须说明是针对某一块还是全部活跃块。'}}
