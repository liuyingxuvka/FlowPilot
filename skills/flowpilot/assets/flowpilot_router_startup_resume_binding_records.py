...
def _normalize_resume_role_agent_records(router: ModuleType, project_root: Path, run_root: Path, run_state: dict[str, Any], payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for raw in iterable:
        ...
        replacement_spawned_after_state_loaded = raw.get('replacement_spawned_after_resume_state_loaded')
        if rehydrated_after_state_loaded is not True:
            raise RouterError(f'{role} must be rehydrated_after_resume_state_loaded=true')
        if result == ROLE_AGENT_REHYDRATION_RESULT and replacement_spawned_after_state_loaded is not True:
            raise RouterError(f'{role} replacement rehydration requires replacement_spawned_after_resume_state_loaded=true')
