...
import flowpilot_router_controller_scheduler_current_work_pending as current_work_pending
...
def _pending_action_has_controller_authority(router: ModuleType, pending: dict[str, Any], controller_ledger: dict[str, Any]) -> bool:
    return current_work_pending._pending_action_has_controller_authority(router, pending, controller_ledger)

def _scheduler_row_for_pending_action(router: ModuleType, run_root: Path, pending: dict[str, Any]) -> dict[str, Any]:
    return current_work_pending._scheduler_row_for_pending_action(router, run_root, pending)
...
def _pending_action_durable_resolution(
    ...
    *,
    ...
    return current_work_pending._pending_action_durable_resolution(
        router,
        run_root,
        pending,
...
def _clear_pending_action_if_durable_wait_resolved(
    ...
    *,
    ...
    return current_work_pending._clear_pending_action_if_durable_wait_resolved(
        router,
        project_root,
        run_root,
        run_state,
...
def _pending_role_wait_should_use_batch_projection(router: ModuleType, pending: dict[str, Any]) -> bool:
    return current_work_pending._pending_role_wait_should_use_batch_projection(router, pending)
...
_OWNER_CHILD_MODULES = (current_work_pending,)
