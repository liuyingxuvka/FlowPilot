# FlowPilot Protocol Kernel Contract

## Core Rule

The project black-box ledger is the truth. Chat memory, a role binding's local
memory, prior run folders, and progress messages are useful context only when they are
...
- Result from a closed role-binding lease.
- Result from a prior route version silently accepted into a current route.
- Worker self-review.
