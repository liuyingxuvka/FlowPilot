# FlowPilot Protocol Kernel Stress Testing

This file describes the deterministic stress layer for the clean FlowPilot
protocol kernel. It is a rehearsal boundary, not a replacement runtime.

The stress layer answers this question:

Can the protocol still protect the project ledger when several parallel
role-binding leases, route versions, reviewers, FlowGuard work orders, and evidence
...
- route mutation with prior packets still visible;
...
prior bad work can be closed or quarantined, a replacement lease can receive a
...
- focused kernel non-authoritative path rejection;
- deterministic multi-round scenarios;
- seeded random long-run checks;
- recorded bad-case replay;
...
The replay pack keeps known prior failure families executable:
...
prior failure modes back into accepted completion evidence.
