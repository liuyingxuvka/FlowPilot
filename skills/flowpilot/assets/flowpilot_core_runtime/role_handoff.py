...
SUBSTANTIVE_WORKSTREAM_INSTRUCTIONS = (
    ...
    "Before execution, understand the bounded assignment and write a specific numbered plan for your own work. You may use bounded delegation, parallel role-local assistance, and role-local FlowGuard when useful, but you must integrate every delegated output yourself.",
    ...
    "In `contract_self_check.workstream_plan_and_completion`, report every numbered step with status, evidence refs, deviations, and unresolved work; also report delegation/integration, verification, and repair. This is a semantic Reviewer contract, not a Runtime mechanical quality score.",
