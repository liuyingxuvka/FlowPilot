...
def _open_role_recovery_transaction(router: ModuleType, project_root: Path, run_root: Path, run_state: dict[str, Any], *, trigger_source: str, recovery_scope: str, target_role_keys: list[str], fault_payload: dict[str, Any]) -> dict[str, Any]:
    _bind_router(router)
    if recovery_scope != 'targeted':
        raise RouterError('role recovery supports only exact targeted scope')
    target_role_keys = router._role_recovery_target_roles(target_role_keys)
    ...
    transaction = {'schema_version': ROLE_RECOVERY_TRANSACTION_SCHEMA, 'transaction_id': transaction_id, 'run_id': run_state['run_id'], 'trigger_source': trigger_source, 'recovery_scope': recovery_scope, 'target_role_keys': list(target_role_keys), 'priority': 'preempt_normal_work', 'normal_work_suspended': True, 'started_at': utc_now(), 'fault_payload': fault_payload, 'active_packet_context': active_packet, 'recovery_ladder': ['restore_old_agent', 'targeted_replacement', 'slot_reconciliation', 'environment_blocked'], 'controller_may_wait_for_normal_work_before_recovery': False, 'controller_may_infer_completion_from_old_agent': False}
...
def _role_recovery_payload_contract(router: ModuleType, run_root: Path, run_state: dict[str, Any], transaction: dict[str, Any]) -> dict[str, Any]:
    ...
    current_results = sorted(result for result in ROLE_RECOVERY_RESULTS if result != ROLE_BINDING_FULL_SET_RECOVERY_RESULT)
    return _payload_contract(name='role_recovery_receipt', required_object='payload', required_fields=required_fields, allowed_values={'runtime_role_assistance_capability_status': ['available'], 'recovery_transaction_id': [str(transaction.get('transaction_id') or '')], 'trigger_source': [str(transaction.get('trigger_source') or '')], 'recovery_scope': [scope], 'target_role_keys': [target_roles], 'recovered_role_bindings[].role_key': target_roles, 'recovered_role_bindings[].model_policy': [ROLE_BINDING_MODEL_POLICY], 'recovered_role_bindings[].reasoning_effort_policy': [ROLE_BINDING_REASONING_EFFORT_POLICY], 'recovered_role_bindings[].recovery_result': current_results, 'recovered_role_bindings[].role_surface_addressable': [True], 'recovered_role_bindings[].current_run_binding_decision': sorted(ROLE_BINDING_CURRENT_RUN_DECISIONS), 'recovered_role_bindings[].rehydrated_for_run_id': [run_state['run_id']], 'recovered_role_bindings[].memory_context_injected': [True], 'recovered_role_bindings[].packet_ownership_reconciled': [True]}, structural_requirements=['Recovery must be recorded before any normal route, packet, gate, or control-blocker work resumes.', 'Attempt old-agent restore before a targeted replacement.', 'If targeted replacement reports capacity_full, reconcile only the requested role slot before reporting environment_blocked.', 'A failed targeted replacement or requested-slot reconciliation must return recovery_result=environment_blocked and must not mark the role binding ready.', 'Recovered or replacement roles must receive exact current-run memory/context before being marked usable.', 'Recovered or replacement roles must have current-run addressable role surfaces before being marked usable.', 'Late output from superseded agent ids must be quarantined and cannot count as packet or gate progress.', 'Packet ownership must be reconciled before PM is asked to continue.', 'Do not submit host-liveness, liveness-probe, bounded-wait, timeout_unknown, full_role_binding, or full-role-recycle fields.'], description='Record the current-run exact-role recovery attempt ladder for a missing or unusable FlowPilot role.', reviewer_check='PM checks this exact-role recovery report before deciding whether to resume, re-dispatch, or escalate.')
