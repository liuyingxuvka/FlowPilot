...
def _resume_role_rehydration_payload_contract(
    ...
    return _payload_contract(
        ...
        allowed_values={
            ...
            "rehydrated_role_bindings[].role_surface_addressable": [True],
            "rehydrated_role_bindings[].current_run_binding_decision": sorted(ROLE_BINDING_CURRENT_RUN_DECISIONS),
        ...
        structural_requirements=[
            ...
            "Reuse an existing current-run role only when its current role surface is addressable after memory/context refresh.",
            "Open replacement bindings from current-run memory when an existing role surface cannot be addressed.",
            ...
            "ACK/progress evidence age is the only current wait authority after a packet is leased; do not submit host-liveness, bounded-wait, or timeout_unknown fields.",
        ],
        optional_fields=[
