...
import flowpilot_router_route_frontier_memory_paths as route_memory_paths
...
def _refresh_route_memory(router: ModuleType, project_root: Path, run_root: Path, run_state: dict[str, Any], *, trigger: str) -> None:
    ...
    map_context = (
        {
            **{key: value for key, value in map_status.items() if key != 'source_ref'},
            'path': map_ref.get('path'),
            'index_only': True,
    ...
    route_nodes = _route_node_history(router, project_root, run_root, route_id or 'route-001', route)
    reviewer_blocks = _event_markers(router, run_state, {'current_node_reviewer_blocks_result', 'reviewer_blocks_node_acceptance_plan'})
    reviewer_passes = _event_markers(router, run_state, {'reviewer_passes_research_direct_source_check', 'reviewer_passes_node_acceptance_plan', 'current_node_reviewer_passes_result', 'reviewer_passes_parent_backward_replay', 'reviewer_passes_evidence_quality_package', 'reviewer_final_backward_replay_passed'})
...
def _require_pm_prior_path_context(router: ModuleType, project_root: Path, run_root: Path, payload: dict[str, Any], *, purpose: str) -> dict[str, Any]:
    ...
    currentness = route_memory_paths._route_memory_currentness(router, run_root, read_json(router.run_state_path(run_root)))
    if currentness.get("current") is not True:
        raise RouterError(
            f"{purpose} requires current route-memory source snapshot: "
            f"{currentness.get('reason') or 'unknown_currentness_failure'}"
        )
    ...
    preserved_source_paths: list[str] = []
    for path in source_paths:
        if path and path not in preserved_source_paths:
            preserved_source_paths.append(path)
    return {'reviewed': True, 'source_paths': preserved_source_paths, 'material_artifact_map_considered': review.get('material_artifact_map_considered'), 'completed_nodes_considered': review.get('completed_nodes_considered') or [], 'superseded_nodes_considered': review.get('superseded_nodes_considered') or [], 'stale_evidence_considered': review.get('stale_evidence_considered') or [], 'prior_blocks_or_experiments_considered': review.get('prior_blocks_or_experiments_considered') or [], 'impact_on_decision': review.get('impact_on_decision'), 'controller_summary_used_as_evidence': False}
