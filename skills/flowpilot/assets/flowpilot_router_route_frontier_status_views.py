    ...
    background_running = [
        {
            ...
            'flow_block_id': str(item.get('flow_block_id') or item.get('run_id') or ''),
            'target_id': str(item.get('target_id') or ''),
            'target_scope': str(item.get('target_scope') or 'single'),
            'operation_target_allowed': bool(item.get('operation_target_allowed')),
            'status': str(item.get('status') or ''),
            'run_root': str(item.get('run_root') or ''),
            'focus_selected': False,
            'background_active': True,
            'stale_residue': bool(item.get('stale_residue')),
        }
        for item in active_ui_task_catalog.get('background_active_tasks', [])
