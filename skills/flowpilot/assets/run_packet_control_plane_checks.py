from __future__ import annotations

import sys
from pathlib import Path

if __package__ in (None, ""):
    sys.path.insert(0, str(Path(__file__).resolve().parent))

from flowguard.explorer import Explorer  # noqa: E402

import packet_control_plane_model as model  # noqa: E402


def main() -> int:
    report = Explorer(
        workflow=model.build_workflow(),
        initial_states=(model.initial_state(),),
        external_inputs=model.EXTERNAL_INPUTS,
        invariants=model.INVARIANTS,
        max_sequence_length=2,
        terminal_predicate=model.terminal_predicate,
        required_labels=(
            "pm_packet_issued",
            "packet_physical_files_written",
            "missing_physical_packet_files_blocked",
            "controller_reminder_checked",
            "controller_missing_pm_reminder",
            "controller_handoff_envelope_only",
            "controller_handoff_missing_mutual_role_reminder_blocked",
            "controller_handoff_body_content_blocked",
            "current_assignment_recorded",
            "major_node_chat_mermaid_displayed_when_cockpit_missing",
            "controller_contamination_returned_to_sender",
            "controller_executes_worker_body_returned_to_sender",
            "private_delivery_returned_to_sender",
            "missing_current_assignment_signature_blocked",
            "unopened_letter_sent_to_pm_for_restart_or_repair",
            "packet_body_open_envelope_receipt_missing_blocked",
            "packet_body_open_ledger_receipt_missing_blocked",
            "router_direct_dispatch_approved",
            "packet_delivered_to_wrong_role_blocked",
            "body_hash_mismatch_blocked",
            "packet_body_hash_identity_mismatch_blocked",
            "stale_packet_body_reused_after_route_mutation_blocked",
            "missing_output_contract_blocked",
            "output_contract_recipient_mismatch_blocked",
            "result_path_escape_blocked",
            "worker_result_envelope",
            "result_without_ledger_absorption_blocked_before_reviewer_relay",
            "current_assignment_delivered_result_envelope_with_holder_status_update",
            "missing_result_current_assignment_signature_blocked",
            "result_mutual_role_reminder_missing",
            "unopened_result_letter_sent_to_pm_for_restart_or_repair",
            "result_body_open_envelope_receipt_missing_blocked",
            "result_body_open_ledger_receipt_missing_blocked",
            "completed_agent_id_role_mapping_blocked",
            "runtime_result_envelope_checked",
            "review_pass_after_role_origin_audit",
            "pm_advanced",
            "controller_origin_artifact_blocked",
            "result_completed_by_wrong_role_blocked",
            "result_body_hash_mismatch_blocked",
            "stale_result_body_reused_after_route_mutation_blocked",
            "router_direct_dispatch_idempotent_current_assignment_reused",
            "closed_lease_result_hard_rejected_before_ledger_result",
            "noncurrent_packet_result_hard_rejected_before_ledger_result",
            "stale_route_result_hard_rejected_before_ledger_result",
            "duplicate_after_accepted_result_hard_rejected_before_ledger_result",
            "duplicate_current_lease_result_hard_rejected_before_ledger_result",
            "pm_repair_required_after_invalid_role_origin",
            "pm_repair_required_after_body_integrity_block",
            "manual_resume_state_loaded",
            "manual_resume_pm_requested",
            "manual_resume_pm_packet_issued",
            "manual_resume_pm_missing_controller_reminder",
            "manual_resume_loaded_worker_result_for_review",
            "manual_resume_ambiguous_worker_locked",
            "manual_resume_missing_state_blocked",
        ),
    ).explore()
    print(report.format_text())
    return 0 if report.ok else 1


if __name__ == "__main__":
    raise SystemExit(main())
