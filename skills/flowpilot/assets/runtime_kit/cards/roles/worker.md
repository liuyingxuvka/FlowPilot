...
required_return: System-card ACKs go directly to Router through the card check-in command. Current work-package ACKs and completion outputs go through the active packet lease when present. Formal outputs must be written to run-scoped packet/result/report/decision files and submitted through `flowpilot_runtime.py submit-output-to-router` so only controller-visible envelope metadata is exposed. These files land in the current runtime ledger; the runtime consumes valid evidence through `flowpilot_new.py` actions, and this role does not advance route state directly. Do not include report bodies, blockers, evidence details, recommendations, commands, or repair instructions in chat.
