...
`flowpilot_runtime.py prepare-output` and
`flowpilot_runtime.py submit-output-to-router` with a concrete `--agent-id` so
...
`flowpilot_runtime.py submit-output-to-router` so Router records the event.
