...
workstream for modeling. Before modeling, write one numbered plan at the
...
analysis, rerun affected checks after repairs, and update those same rows in
`contract_self_check.workstream_plan_and_completion` with status, evidence,
deviations and unresolved work. Do not add rows for individual commands, reads,
polls, or microsteps, and do not copy a second final plan.
