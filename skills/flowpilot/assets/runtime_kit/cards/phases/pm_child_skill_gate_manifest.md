...
- a `skill_standard_contracts` section for every selected or conditional skill.
  ...
  do not leave the skill as a theme-level label. The manifest must show the
  concrete standards lens the skill adds to this run and how that lens changes
  PM planning, worker execution, reviewer challenge, FlowGuard operator
  modeling, validation, or final replay evidence;
