...
- coverage for runtime-requested role bindings or an explicit blocked-capability path;
- continuation mode evidence bound to the user's startup answer;
- fresh current-task role-binding evidence or same-task memory reuse/replacement evidence;
...
- Approval: `flowpilot_new.py open-packet --output-type pm_startup_activation_approval --role project_manager --agent-id <agent-id>`, then `flowpilot_new.py submit-result --lease-id <lease-id> --packet-id <packet-id> --body <sealed_result_summary>`; Router records `pm_approves_startup_activation`.
