...
Do this only for a parent/module whose own node-entry gate is current:
accepted `node_acceptance_plan` plus current `node_context_package` for that
same node id and repair generation. Do not use a child plan or child context
package as the parent/module entry evidence.
...
FlowGuard operator process model missed that class of work. When it did, update the
