...
Treat any selected ordinary child skill as a standards lens inside the same
FlowPilot route, not a task-family route split. The selection must name the
specific standard the skill contributes, the role that will use it, the
existing route/node/review artifact where it will be projected, and the
...
`project_manager`, `human_like_reviewer`, `flowguard_operator`,
`flowguard_operator`, or `worker`; the `use_context`
