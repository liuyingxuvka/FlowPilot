...
When the blocker is semantic dilution, generic acceptance wording, or loss of
source intent, PM owns the repair. Restore the concrete source-intent
acceptance row, the affected `acceptance_item_id`, the current artifact or
node slice that must change, and the same Reviewer gate that must recheck it.
A PM explanation that the artifact is "good enough" does not close this class
of blocker without a repaired artifact, contract, node plan, or evidence row.
...
--body-file <sealed_result_body_file>` with the same lease and packet id. Do not
