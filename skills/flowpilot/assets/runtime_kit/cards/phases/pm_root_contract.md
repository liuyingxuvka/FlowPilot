...
Before freezing, confirm the run has clean `startup` and
`product_architecture` entries in `self_interrogation_index.json`. Router will
reject root contract freeze if either scope is missing, malformed, or declares
an unresolved hard/current self-interrogation finding.
...
objects, actions, quality floor, quantities, constraints, and prohibitions that
the user actually supplied. If the source request is ambiguous, preserve the
