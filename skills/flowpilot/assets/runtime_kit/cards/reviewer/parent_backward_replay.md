...
This review is valid only after the runtime has already accepted the same
parent/module node's own `node_acceptance_plan` and current
`node_context_package`. Do not pass by borrowing a child plan, child context
package, terminal ledger row, or PM prose as parent/module entry evidence.
