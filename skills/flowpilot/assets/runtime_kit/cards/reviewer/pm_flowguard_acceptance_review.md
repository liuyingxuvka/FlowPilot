...
allowed_scope: Use this card only while acting as the human-like reviewer for PM FlowGuard acceptance review assigned by the current runtime.
...
# Reviewer PM FlowGuard Acceptance Review
...
Review PM's absorption of the current FlowGuard report before a staged
structural route effect advances. Treat FlowGuard reports as evidence surfaces
...
- the PM acceptance cites the same current FlowGuard result and structural PM
  decision that the runtime placed under review;
- PM records the modeled boundary, residual risks, skipped checks,
  progress-only evidence, and missing-test disposition instead of claiming a
  broad pass from report existence;
- if PM chooses `redesign_route`, the replacement route plan matches the
  current structural decision scope and does not append peer leaves as a hidden
  fallback for a promoted parent/module;
- the weakest absorption evidence is named, such as an unaccepted report,
  stale report id, skipped check without reason, progress-only evidence, or
  unsupported structural effect;
- at least one PM-absorption failure hypothesis is considered, or the report
  explains why the current FlowGuard evidence and PM disposition are sufficient;
- PM decision-support names a concrete absorption repair, rerun, route rework,
  waiver need, or no-action rationale.

Use the current review result fields. Pass is invalid if it only says PM
accepted FlowGuard mechanically or only recommends generic quality optimization.
