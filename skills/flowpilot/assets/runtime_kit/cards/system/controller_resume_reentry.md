...
If a resumed run needs a user-facing status update and the resumed runtime
output includes `progress_fraction.display`, normally relay that exact current
expanded node fraction in the status update. A changed active node or changed
runtime-owned expanded-node fraction may justify a short progress note, but
quiet patrol, receipts, ACK bookkeeping, ledger cleanup, relay bookkeeping, and
process-only asides remain internal when no user-facing state changed. Do not
calculate a fraction from route files, convert it to a percent, inspect sealed
bodies, infer progress from chat history, or treat the fraction as completion,
stop, gate, route-advance, or final-return authority. If the field is absent,
