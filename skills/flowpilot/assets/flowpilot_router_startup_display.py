...
def _display_surface_receipt_from_payload(router: ModuleType, payload: dict[str, Any], *, run_id: str, requested: str, selected_surface: str) -> dict[str, Any]:
    ...
    if receipt is None:
        actual = 'display_blocked' if selected_surface == 'cockpit' else selected_surface
        return {'schema_version': DISPLAY_SURFACE_RECEIPT_SCHEMA, 'run_id': run_id, 'requested_display_surface': requested, 'actual_surface': actual, 'source_kind': 'controller_user_dialog_render', 'host_display_surface_verified': False, 'display_blocked': actual == 'display_blocked', 'recorded_at': utc_now()}
    ...
    if actual not in {'chat_route_sign', 'cockpit', 'display_blocked'}:
        raise RouterError('display_surface_receipt.actual_surface must be chat_route_sign, cockpit, or display_blocked')
    ...
    if selected_surface == 'cockpit' and actual not in {'cockpit', 'display_blocked'}:
        raise RouterError('Cockpit request cannot be satisfied by a chat display surface')
    if selected_surface == 'chat_route_sign' and actual != 'chat_route_sign':
        raise RouterError('chat display request must use actual_surface=chat_route_sign')
    return {'schema_version': DISPLAY_SURFACE_RECEIPT_SCHEMA, 'run_id': run_id, 'requested_display_surface': requested, 'actual_surface': actual, 'source_kind': str(receipt.get('source_kind') or 'host_receipt'), 'host_display_surface_verified': bool(receipt.get('host_display_surface_verified')), 'display_blocked': actual == 'display_blocked', 'host_surface_id': receipt.get('host_surface_id'), 'notes': receipt.get('notes'), 'recorded_at': utc_now()}
...
def _write_display_surface_status(router: ModuleType, project_root: Path, run_root: Path, run_state: dict[str, Any], display_confirmation: dict[str, Any], payload: dict[str, Any] | None=None) -> None:
    ...
    selected_surface = 'cockpit' if 'cockpit' in requested_normalized else 'chat_route_sign'
    ...
    elif actual_surface == 'display_blocked':
        selected_surface = 'display_blocked'
    else:
        selected_surface = 'chat_route_sign'
    chat_displayed = selected_surface == 'chat_route_sign'
    route_sign = router._startup_route_sign_payload(project_root, write=True, mark_chat_displayed=chat_displayed)
    ...
    write_json(run_root / 'display' / 'display_surface.json', {'schema_version': 'flowpilot.display_surface.v1', 'run_id': run_state['run_id'], 'requested_display_surface': requested, 'selected_surface': selected_surface, 'actual_display_surface': actual_surface, 'chat_route_sign_path': project_relative(project_root, sign_path), 'standard_route_sign_markdown_path': project_relative(project_root, Path(route_sign['markdown_preview_path'])), 'standard_route_sign_mermaid_path': project_relative(project_root, Path(route_sign['mermaid_path'])), 'standard_route_sign_display_packet_path': project_relative(project_root, Path(route_sign['display_packet_path'])), 'route_sign_mermaid_sha256': route_sign['mermaid_sha256'], 'chat_display_required': bool(route_sign['chat_display_required']) and chat_displayed, 'chat_displayed_by_controller': chat_displayed, 'user_dialog_display_confirmation': display_confirmation, 'display_surface_receipt': display_receipt, 'host_display_surface_verified': bool(display_receipt.get('host_display_surface_verified')), 'generated_files_alone_satisfy_chat_display': False, 'controller_display_rule': 'Controller must use the requested display surface and write a matching receipt; generated files alone do not satisfy display.', 'cockpit_status': 'host_verified_open' if selected_surface == 'cockpit' else ('blocked_unverified' if selected_surface == 'display_blocked' else 'not_requested'), 'cockpit_probe_required_for_requested_cockpit': 'cockpit' in requested_normalized, 'display_surface_blocked': selected_surface == 'display_blocked', 'display_block_reason': 'cockpit_unavailable_or_unverified' if selected_surface == 'display_blocked' else '', 'updated_at': utc_now()})
...
def _next_startup_display_action(router: ModuleType, project_root: Path, run_state: dict[str, Any], run_root: Path) -> dict[str, Any] | None:
    ...
    return make_action(action_type='write_display_surface_status', actor='controller', label='controller_writes_startup_display_surface_status', summary='Write the requested startup display-surface status before PM startup work.', allowed_reads=[project_relative(project_root, run_root / 'startup_answers.json'), project_relative(project_root, run_root / 'execution_frontier.json')], allowed_writes=[project_relative(project_root, run_root / 'display' / 'display_surface.json'), project_relative(project_root, run_root / 'diagrams' / 'current_route_sign.md'), project_relative(project_root, run_root / 'diagrams' / 'user-flow-diagram.mmd'), project_relative(project_root, run_root / 'diagrams' / 'user-flow-diagram.md'), project_relative(project_root, run_root / 'diagrams' / 'user-flow-diagram-display.json'), project_relative(project_root, run_root / 'display' / 'user_dialog_display_ledger.json'), project_relative(project_root, router.run_state_path(run_root))], extra={'postcondition': 'startup_display_status_written', **display_gate, 'chat_display_required': route_sign['chat_display_required'] and not cockpit_requested, 'route_sign_mermaid_sha256': route_sign['mermaid_sha256'], 'requested_display_surface': requested_display_surface, 'resolved_display_surface': 'cockpit-required' if cockpit_requested else 'chat-requested', 'cockpit_probe_required_for_requested_cockpit': cockpit_requested, 'display_surface_blocks_if_unverified': cockpit_requested, 'payload_contract': _display_surface_receipt_payload_contract()})
