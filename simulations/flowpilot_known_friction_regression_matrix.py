...
KNOWN_FRICTION_ROWS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "historical_bad_case": "A daemon role-output ledger replay sees the same PM package disposition identity with a different body_hash after the conflict is already owned by a control blocker or PM repair decision.",
        "trigger_state": "A PM package disposition for the same batch/generation is already recorded, a stale conflicting role-output envelope remains replayable, and control-plane ownership already points at a blocker, repair transaction, or terminal quarantine.",
        "expected_safe_behavior": "Router classifies the stale conflict, preserves the legal wait or terminal quarantine, writes audit evidence, never accepts the stale body as success, and keeps the daemon live.",
        "model_obligation": "event_idempotency.package_conflict_repair_owned_replay + control_plane_friction.pm_package_repair_owned_conflict_replay",
        ...
        "evidence_current": True,
        "evidence_role": PRIMARY_ROLE,
        "full_confidence_boundary": "Covers known repair-owned package-disposition body_hash replay branches for material, research, and current-node package families; does not prove unbounded daemon soak or live AI semantic quality without fresh historical replay and daemon artifacts.",
        "live_ai_semantic_quality_proven": False,
    },
    {
