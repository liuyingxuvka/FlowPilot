...
SCRIPT_RESULT_PATHS = {
    ...
    "run_flowpilot_current_status_projection_checks.py": "simulations/flowpilot_current_status_projection_results.json",
    ...
    "run_flowpilot_core_runtime_checks.py": "simulations/flowpilot_core_runtime_results.json",
}
...
REQUIRED_FRICTION_IDS = {
    ...
    "known_friction.producerless_repair_continuation",
...
REQUIRED_SOURCE_CLASSES = {
    ...
    "producerless_repair_transaction",
...
KNOWN_FRICTION_ROWS: tuple[dict[str, Any], ...] = (
    {
        ...
        "historical_bad_case": "A worker current-node result body has a Contract Self-Check heading but misses required machine fields.",
        "trigger_state": "A current_node batch is results_relayed_to_pm and PM must decide whether every worker result satisfies the source output contract.",
        "expected_safe_behavior": "PM disposition records rework without reviewer release, status names the failed self-check, and repair remains on the current work-packet path.",
        ...
        "runtime_surface": "role_output_runtime + packet_runtime + current_node PM disposition writer",
        "runtime_test": "tests.test_flowpilot_output_contracts.FlowPilotOutputContractTests.test_contract_self_check_metadata_reports_current_node_worker_missing_fields",
    ...
    },
    {
        ...
        "runtime_test": "tests.router_runtime.control_blockers.ControlBlockersRuntimeTests.test_pm_repair_decision_state_persists_before_followup_wait_is_exposed",
        "replay_fixture": "known_friction.pm_repair_decision_persists_before_current_followup_wait",
        "child_evidence_ids": [
            "historical_live_run_replay_matrix",
    ...
    {
        "friction_id": "known_friction.producerless_repair_continuation",
        "priority": "P0",
        "source_class": "producerless_repair_transaction",
        "historical_bad_case": "A repair submits retired replacement_packets or waits for an event without a concrete current producer.",
        "trigger_state": "A current control blocker repair proposes role_reissue, await_existing_event, or operation_replay after a rejected result.",
        "expected_safe_behavior": "Router rejects retired replacement-packet authority and producerless waits; corrected repair converges through a registered current rerun target or safe operation replay.",
        "model_obligation": "repair_transactions.current_repair_requires_concrete_producer",
        "model_check": "python simulations/run_flowpilot_repair_transaction_checks.py",
        "runtime_surface": "control-blocker repair transaction validation + Router next-action projection",
        "runtime_test": "tests.router_runtime.control_blockers.ControlBlockersRuntimeTests.test_delivered_control_blocker_with_empty_repair_transaction_requires_pm_repair_decision",
        "replay_fixture": "control_blocker.producerless_role_reissue_then_registered_current_repair",
        "child_evidence_ids": [
            "e2e_synthetic_chaos_matrix",
            "real_router_dry_run_rehearsal_matrix",
            "historical_live_run_replay_matrix",
        ...
        "forbidden_shortcuts": [
            "accept_retired_replacement_packets",
    ...
    },
    {
        ...
        "model_obligation": "current_status_projection.current_router_facts_drive_display",
        "model_check": "python simulations/run_flowpilot_current_status_projection_checks.py",
    ...
    },
    {
        ...
        "historical_bad_case": "run-20260527-212331 had a valid runtime-role rehydration report, but a later receipt replay did not restore resume_roles_restored and reopened a mechanical control blocker.",
        "trigger_state": "Heartbeat/manual resume sees a done rehydrate_role_bindings receipt or report while run-state flags are stale or incomplete.",
        "expected_safe_behavior": "Router validates the current-run role binding recovery report, reclaims the resume postcondition idempotently, and only opens a blocker when current-run evidence is absent or invalid.",
        ...
        "replay_fixture": "run-20260527-212331.role_binding_recovery_report_ready_but_resume_flags_false",
        "child_evidence_ids": [
            "historical_live_run_replay_matrix",
            "current_transcript_regression",
        ...
        "forbidden_shortcuts": [
            "ignore_current_run_role_binding_recovery_report",
            "open_rehydrate_blocker_before_replay_scan",
        ],
        "evidence_status": "passed",
        "evidence_current": True,
        "evidence_role": PRIMARY_ROLE,
        "full_confidence_boundary": "Covers current-run runtime-role report replay and invalid-report blocking; does not prove host agents stay alive forever after rehydration.",
        "live_ai_semantic_quality_proven": False,
    },
    {
    ...
    },
    {
        ...
        "historical_bad_case": "run-20260530-102304 accepted reviewer result-0003, then a repeated wait created replacement lease-0004 and regressed packet-0003 to acknowledged. run-20260531-210441 then showed the same health family at larger scale: accepted-packet stale active leases, heavy status projection, under-specified recovery duty, node context package shape drift, evidence-summary instability, and stale terminal pointer display.",
        "trigger_state": "New FlowPilot foreground duty is waiting on active packet leases, a later pass may replace or reassign the same packet, and terminal/status surfaces may try to summarize a long run while stale accepted-packet lease health still exists.",
        "expected_safe_behavior": "Patrol keeps Controller on timed wait while progress/liveness is healthy; replacement requires current failure evidence; packets with accepted_result_id cannot be reassigned or ACK-regressed; accepted-result races are repaired by closing mistaken replacement leases and restoring the original accepted result; default status is compact and body-free; recovery duty names a concrete next command; terminal current status follows final preflight authority.",
        ...
        "child_evidence_ids": [
            ...
            "core_runtime_run_20260531_210441_health_family",
