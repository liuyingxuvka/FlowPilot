...
HAZARD_EXPECTED_FAILURES = {
    ...
    model.RETIRED_DIRECT_EVENT_COMPETES_WITH_PM_PACKET: "unsupported direct FlowGuard operator event competes with PM role-work result contract",
    ...
    model.RETIRED_EVENT_ACCEPTED_WITHOUT_REQUIRED_GATE_FLAG: (
...
def _state_id(state: model.State) -> str:
    return (
        ...
        f"product_model={state.pm_acceptance_required_after_submission},{state.pm_acceptance_completed},"
        f"{state.downstream_product_flow_allowed},"
        f"{state.canonical_product_behavior_model_artifact_written}|"
        f"process_model={state.pm_process_acceptance_required_after_submission},{state.pm_process_acceptance_completed},"
...
def _candidate_fix_plan() -> dict[str, object]:
    return {
        ...
        "minimum_runtime_change_set": [
            ...
            "Use product behavior model submission as the only FlowGuard operator product-scope gate completion path.",
            "Write only the canonical product behavior model artifact for new runs.",
            "Require PM product behavior model acceptance before reviewer challenge or route planning can use the model.",
            "Use process route model submission as the only FlowGuard operator route-scope route gate completion path.",
            "Write only the canonical process route model artifact for new runs.",
        ...
        "accepted_runtime_shapes": [
            ...
            model.VALID_PRODUCT_BEHAVIOR_MODEL_SUBMISSION_WITH_CANONICAL_ARTIFACT,
            model.VALID_PROCESS_ROUTE_MODEL_SUBMISSION_WITH_CANONICAL_ARTIFACT,
        ],
    }
