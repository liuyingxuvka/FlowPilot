"""FlowGuard model for FlowPilot role-recovery liveness proof gates."""
...
MODEL_ID = "flowpilot_role_recovery_liveness_proofs"
...
def next_safe_states(state: State) -> Iterable[Transition]:
    ...
    if _role_recovery_proof_is_current(state):
        yield Transition("classify_current_recovery_liveness_proven", replace(state, status="safe", classification="safe"))
        return
...
def _hard_check_failures(state: State) -> list[str]:
    ...
    if state.status == "safe" and not state.daemon_error_type and not _role_recovery_proof_is_current(state):
        failures.append("role recovery was marked safe without current transaction and active host proof")
...
INVARIANTS = (
    Invariant(
        "flowpilot_role_recovery_liveness_proof_contract",
        "Current role recovery readiness requires current transaction and active host proof.",
        hard_invariant,
    ),
)
...
class RoleRecoveryLivenessStep:
    """Input x State -> Set(Output x State) for role recovery liveness proof."""

    name = "RoleRecoveryLivenessStep"
    reads = ("transaction_id", "role_slot", "host_liveness", "daemon_error")
...
def build_workflow() -> Workflow:
    return Workflow((RoleRecoveryLivenessStep(),), name=MODEL_ID)


def hazard_states() -> dict[str, State]:
    return {
        "stale_report_marked_safe": replace(_selected_state(STALE_REPORT_RECLAIM), status="safe"),
        "unknown_liveness_marked_safe": replace(_selected_state(UNKNOWN_LIVENESS), status="safe"),
