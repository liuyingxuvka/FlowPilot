"""FlowGuard model for FlowPilot unsupported-transition pruning.
...
class Candidate:
    """One scanned unsupported_historical/fallback/alias occurrence."""
    ...
    text: str
    surface: str
    has_unsupported_historical_term: bool = False
    has_historical_term: bool = False
    has_unsupported_term: bool = False
...
class UnsupportedTransitionPruningStep:
    """Classify one unsupported-transition pruning candidate.

    Input x State -> Set(Output x State)
    reads: candidate surface, prior-path terms, acceptance/rejection wording,
    ...
    name = "UnsupportedTransitionPruningStep"
    reads = (
        "candidate_surface",
    ...
    input_description = "unsupported-transition source occurrence"
...
def next_states(input_obj: Candidate, state: State) -> tuple[Transition, ...]:
    ...
    if (
        candidate.has_unsupported_historical_term
        or candidate.has_historical_term
        or candidate.has_unsupported_term
...
def build_workflow() -> Workflow:
    return Workflow((UnsupportedTransitionPruningStep(),), name="flowpilot_unsupported_transition_pruning")
