"""Run checks for the FlowPilot reviewer active-challenge model."""
...
import flowpilot_reviewer_active_challenge_model as model


RESULTS_PATH = Path(__file__).resolve().with_name("flowpilot_reviewer_active_challenge_results.json")
...
HAZARD_EXPECTED_FAILURES = {
    ...
    model.NO_CHALLENGE_ACTIONS: "reviewer pass lacks recorded challenge actions",
    model.GENERIC_ACTIONS_NOT_TASK_SPECIFIC: "reviewer challenge actions do not fit the task family",
    model.NO_DIRECT_EVIDENCE_OR_WAIVER: "reviewer pass lacks direct evidence or an approved waiver",
    model.HARD_ISSUE_DOWNGRADED_TO_RESIDUAL: "reviewer did not block a hard requirement or core commitment failure",
...
def _state_id(state: model.State) -> str:
    return f"scenario={state.scenario}|status={state.status}|task={state.task_family}|reason={state.terminal_reason}"
...
def _hazard_report() -> dict[str, object]:
    ...
    for name, state in model.hazard_states().items():
        challenge_failures = model.reviewer_challenge_failures(state)
        expected = HAZARD_EXPECTED_FAILURES[name]
        detected = any(expected in failure for failure in challenge_failures)
        hazards[name] = {
            "detected": detected,
