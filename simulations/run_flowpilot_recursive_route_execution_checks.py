"""Run FlowGuard checks for recursive route execution in the new FlowPilot runtime."""
...
try:  # pragma: no cover
    from . import flowpilot_recursive_route_execution_model as model
except ImportError:  # pragma: no cover
    import flowpilot_recursive_route_execution_model as model


ROOT = Path(__file__).resolve().parent
RESULTS_PATH = ROOT / "flowpilot_recursive_route_execution_results.json"
...
def run_checks() -> dict[str, Any]:
    ...
    rows = [
        {
            "id": "recursive_route_execution_flowguard_model",
            "status": "passed" if flowguard["ok"] else "failed",
            "freshness": "current",
            "scope": "routine",
            "evidence": ["simulations/flowpilot_recursive_route_execution_model.py"],
        },
        {
            "id": "recursive_route_target_plan",
            "status": "passed" if target_plan["ok"] else "failed",
            "freshness": "current",
            "scope": "routine",
            "evidence": ["openspec/changes/restore-recursive-route-execution-runtime/tasks.md"],
        },
        {
            "id": "recursive_route_hazard_replay",
    ...
    return {
        "result_type": "flowpilot_recursive_route_execution_checks",
