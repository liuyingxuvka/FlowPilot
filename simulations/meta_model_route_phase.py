...
def apply_route_phase(self, state: State) -> Iterable[FunctionResult]:
    ...
    if (
        ...
        yield _step(
            ...
            active_node="startup_runtime_mechanical_audit",
        )
        return
    ...
    if (
        ...
        yield _step(
            ...
            action="Runtime/Router mechanically checks sealed startup intake, current run identity, prior-work boundary, display/status evidence, and current requested-responsibility role binding freshness, then records structured blockers for PM without releasing route work",
            ...
            active_node="pm_interprets_startup_runtime_audit",
        )
        return
    ...
    if (
        ...
        yield _step(
            ...
            action="PM opens the sealed user_intake packet, records the initial node and work-package decision, and verifies runtime mechanical evidence paths before route work is released",
            pm_startup_intake_node_package_decision_recorded=True,
            startup_pm_capability_resolution_recorded=True,
            active_node="pm_startup_intake_decision",
        )
        return
    ...
    if (
        ...
        yield _step(
            ...
            action="project manager releases startup intake into the initial route node/work package from the current clean runtime mechanical audit",
