"""Run stress checks for the clean FlowPilot protocol kernel."""
...
try:  # pragma: no cover
    from . import flowpilot_protocol_kernel_stress_model as model
    from . import run_flowpilot_protocol_kernel_checks as kernel_runner
except ImportError:  # pragma: no cover
    import flowpilot_protocol_kernel_stress_model as model
    import run_flowpilot_protocol_kernel_checks as kernel_runner


ROOT = Path(__file__).resolve().parent
RESULTS_PATH = ROOT / "flowpilot_protocol_kernel_stress_results.json"
    ...
    *,
    ...
    rows = [
        _row(
            ...
            "current",
            "routine",
            ["simulations/run_flowpilot_protocol_kernel_checks.py", "simulations/flowpilot_protocol_kernel_results.json"],
...
def run_checks(
    *,
    ...
    report = {
        "result_type": "flowpilot_protocol_kernel_stress",
        "model_id": model.MODEL_ID,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "mode": "release" if release_evidence else "routine",
        "kernel_unsupported_historical": {
            "ok": kernel["ok"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"[flowpilot-protocol-kernel-stress] ok={report['ok']} mode={report['mode']}")
