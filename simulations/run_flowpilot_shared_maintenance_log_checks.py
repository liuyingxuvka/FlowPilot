"""Run checks for the FlowPilot shared skill maintenance log model."""
...
import flowpilot_shared_maintenance_log_model as model


RESULTS_PATH = Path(__file__).resolve().with_name("flowpilot_shared_maintenance_log_results.json")
...
HAZARD_EXPECTED_FAILURES = {
    model.FLOWPILOT_PRIVATE_LOG: "maintenance row is not written to a shared Spark-style log",
...
def _hazard_report() -> dict[str, object]:
    ...
    for name, state in model.hazard_states().items():
        bookkeeping_failures = model.bookkeeping_failures(state)
        expected = HAZARD_EXPECTED_FAILURES[name]
        detected = any(expected in failure for failure in bookkeeping_failures)
        hazards[name] = {
            "detected": detected,
            "expected_failure": expected,
            "failures": bookkeeping_failures,
        }
        if not detected:
