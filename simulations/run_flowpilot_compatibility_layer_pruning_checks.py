"""Run FlowGuard checks for FlowPilot compatibility-layer pruning."""
...
import flowpilot_compatibility_layer_pruning_model as model


ROOT = Path(__file__).resolve().parent
PROJECT_ROOT = ROOT.parent
RESULTS_PATH = ROOT / "flowpilot_compatibility_layer_pruning_results.json"
...
TERM_RE = re.compile(
    r"\b(compatib\w*|legacy|retired|alias(?:es)?|fallback|old)\b",
    re.IGNORECASE,
)

ACCEPTANCE_RE = re.compile(
    r"accepted alias|accepted aliases|accepts? .*alias|alias(?:es)? .*accept|"
    r"old inputs? .*accepted|accepted .*old|legacy .*accepted|accepted .*legacy|"
    r"retired .*accepted|accepted .*retired|canonicali[sz]ed retired|"
    r"migrated retired|compatibility aliases|compatibility-only alias|"
    r"preserve .*old import path|remains? compatible|remain compatible|"
    r"compatibility facade|compatibility entrypoint|compatibility exports?|"
    r"compatibility layer|legacy entrypoint|legacy import surface|legacy full",
    re.IGNORECASE,
)
...
PUBLIC_FACADE_RE = re.compile(
    r"compatibility facade|public entrypoint|public entrypoints|compatibility entrypoint|"
    r"compatibility exports?|import path|legacy import surface|"
...
RUNTIME_ACCEPTANCE_SYMBOL_RE = re.compile(
    ...
    r"compat_output_alias|legacy_.*accepted|accepted_.*legacy",
    re.IGNORECASE,
)
...
def _candidate_from_line(path: Path, line_no: int, text: str) -> model.Candidate | None:
    ...
    is_current_safety_fallback = "fallback" in lower and bool(SAFETY_FALLBACK_RE.search(text)) and not (
        "legacy" in lower or "retired" in lower or "alias" in lower or "compatib" in lower or "old" in lower
    )
    ...
    return model.Candidate(
        ...
        text=text.strip(),
        surface=surface,
        has_compatibility_term="compatib" in lower,
        has_legacy_term="legacy" in lower,
        has_retired_term="retired" in lower,
...
def _build_graph(candidates: tuple[model.Candidate, ...]) -> dict[str, object]:
    ...
    inputs = candidates or (
        model.Candidate(
            ...
            path="synthetic",
            line=0,
            text="no compatibility candidates",
            surface="synthetic",
        ),
    )
...
def run_checks() -> dict[str, object]:
    ...
    return {
        ...
        "model": "flowpilot_compatibility_layer_pruning",
