...
REQUIRED_REHEARSAL_IDS = {
    ...
    "shadow.upgrade_migration.legacy_pointer_install_freshness",
...
SHADOW_LAUNCHER_CHAOS_ROWS: tuple[dict[str, Any], ...] = (
    ...
    {
        "rehearsal_id": "shadow.upgrade_migration.legacy_pointer_install_freshness",
        "surface": "upgrade_migration",
        "phase_sequence": ["legacy_current_pointer", "state_load", "installed_source_presence"],
        "entrypoints": ["state", "load_bootstrap_state", "load_run_state", "installed_skill_files"],
        "fake_ai_artifacts": ["legacy_current_pointer", "minimal_running_run"],
        "failure_injection": "old_active_run_pointer_fields_and_existing_installed_skill",
        "expected_standard_state": "legacy_active_pointer_resolves_to_current_run_state_without_fallback_to_unscoped_state",
        "protected_state_invariant": "old pointers may be read only as compatibility input and cannot bypass current run root authority",
        "required_evidence": [
            "active_run_root_compatibility_loaded",
            "run_state_matches_legacy_pointer_run",
            "installed_skill_assets_present",
        ],
        "forbidden_shortcuts": ["fallback_to_legacy_dot_flowpilot_root", "assume_install_fresh_without_file_evidence"],
        "finite_package_classes": ["legacy_pointer_shadow_package"],
        "soak_cycle_bound": 1,
        "evidence_test": (
            "FlowPilotShadowLauncherChaosReplayTests."
        ...
        "live_ai_semantic_quality_proven": False,
        "destructive_live_state_mutation": False,
        "confidence_boundary": "does not prove every historical schema migration",
    },
    {
