"""Run FlowGuard checks for the new FlowPilot formal entrypoint."""
...
try:  # pragma: no cover
    from . import flowpilot_new_entrypoint_model as model
except ImportError:  # pragma: no cover
    import flowpilot_new_entrypoint_model as model


ROOT = Path(__file__).resolve().parent
RESULTS_PATH = ROOT / "flowpilot_new_entrypoint_results.json"
...
def _target_plan_report() -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_role": "new_entrypoint_target_path_not_live_user_proof",
        "failures": failures,
...
def run_checks() -> dict[str, Any]:
    ...
    rows = [
        {
            "id": "new_entrypoint_flowguard_model",
            "status": "passed" if flowguard["ok"] else "failed",
            "freshness": "current",
            "scope": "routine",
            "evidence": ["simulations/flowpilot_new_entrypoint_model.py"],
        },
        {
            "id": "new_entrypoint_target_plan",
            "status": "passed" if target_plan["ok"] else "failed",
            "freshness": "current",
            "scope": "routine",
            "evidence": ["openspec/changes/generate-new-flowpilot-formal-entrypoint/tasks.md"],
        },
        {
            "id": "new_entrypoint_hazard_replay",
    ...
    return {
        "result_type": "flowpilot_new_entrypoint_checks",
